...
In the <<chapter_05_post_and_database>> we saw how just blundering ahead without thinking
...
Something like <<multiple-lists-users-and-urls>>:
...
This is much more widely applicable rule for programming in business, actually.
...
Our exisisting FT, `test_can_start_a_todo_list()`,
is going to add as our regression test.

Let's translate our scratchpad into a new functional test method,
...
2. _Move_ the method called `test_displays_all_list_items()`.
...
and in a Django app, that's often a template problem.
...
(lists.tests.HomePageTest.test_renders_input_form)
 ---------------------------------------------------------------------
Traceback (most recent call last):
  File "...goat-book/superlists/lists/tests.py", line 13, in
...
value="Kk5XLzXT584jFJprzx1uJgahBzrApWcyeK2HFq5342AfdhZVjidWGYqzrAwpuuzW">\n
</form>\n    <table id="id_list_table">\n      \n    </table>\n
</body>\n</html>\n'

======================================================================
FAIL: test_renders_input_form (lists.tests.ListViewTest.test_renders_input_form)
[...]
AssertionError: False is not true : Couldn't find '<form method="POST" action="/">' in the following response
b'<html>\n  <head>\n    <title>To-Do lists</title>\n  </head>\n  <body>\n
[...]
----
...
In the _Red/Green/Refactor_ dance, our unit tests pass
...
A blank template, which gives us two error--good to know the tests are
...
since I think that shows nice and clearly what we need to modify:
...
----
  File "...goat-book/superlists/functional_tests/tests.py", line 96, in
test_multiple_users_can_start_lists_at_different_urls
    self.wait_for_row_in_list_table("1: Buy milk")
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^^^^^^^^^^^^^^^
[...]
...
Great!  Only 1 failure, so we know our regression test (the first FT)
is passing.
...
<2> When Francis loads the homepage, there's no sign of Edith's list
...
Open up _lists/tests.py_, create a new class called `NewListTest`, and
_move_ the three tests to do with new list creation into it:

1. `test_can_save_a_POST_request()`
2. `test_redirects_after_POST()`
3. test_only_saves_items_when_necessary()

Then, change the URL they point to:
...
I don't think this is a very common convention any more these days,
but I quite like it.
...
Then we get 3 errors saying
"The view lists.views.new_list didn't return an HttpResponse object".
(This is getting rather familiar!)
We could return a raw `HttpResponse`,
but since we know we'll need a redirect,
let's borrow a line from `home_page`:
...
That gives us a perhaps-slightly-unexpected failure.
I'll strip it down below to show the relevant parts,
hopefully you're now getting used to the unexpected-error-debugging
steps:
...
----
======================================================================
ERROR: test_only_saves_items_when_necessary  <2>
(lists.tests.NewListTest.test_only_saves_items_when_necessary)
 ---------------------------------------------------------------------
Traceback (most recent call last):
  [...]
  File "...goat-book/superlists/lists/tests.py", line 29, in
test_only_saves_items_when_necessary
    self.client.get("/lists/new")  <3>
    ~~~~~~~~~~~~~~~^^^^^^^^^^^^^^
  [...]
  File "...goat-book/superlists/lists/views.py", line 14, in new_list
    Item.objects.create(text=request.POST["item_text"])  <4>
                             ~~~~~~~~~~~~^^^^^^^^^^^^^
[...]
    raise MultiValueDictKeyError(key)
django.utils.datastructures.MultiValueDictKeyError: 'item_text'  <1>
...
<1> Check the error
<2> Check which test failed. Ah, it's our one about not saving items
    unnecessarily.
<3> Check where the error comes from in our tests: it's when
    do a `self.client.get()` request
<4> Finally check the line in our code that's causing the problem,
    it's when we try and look into `request.POST[]`.
    Bearing in mind we now know we're in a get request,
    that won't work.

Let's add a little guard in our view:


[role="sourcecode"]
.lists/views.py (ch07l024-2)
====
[source,python]
----
def new_list(request):
...
What response _should_ we give for a GET request?
This view is only supposed to handle POST requests.
We could return an error of some kind,



[role="sourcecode"]
.lists/views.py (ch04l004)
====
[source,python]
----
...
and have a look around with a browser and the dev tools,
to see what's going on.

* First I tried submitting a new list item,
  and saw we get sent back to the home page.

* Then I did the same with the browser dev tools open,
...
Actually both our forms are still pointing to the old URL.
...
[subs="specialcharacters,macros"]
----
  File "...goat-book/lists/tests.py", line 20, in test_redirects_after_POST
...
That feels like we're done with migrating to the new URL structure;
...
// IDEA (seb): maybe rename correct_list to current_list? Personally, I have an allergy
