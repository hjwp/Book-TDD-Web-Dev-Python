...
class SharingTest(FunctionalTest):
    def test_can_share_a_list_with_another_user(self):
        ...
        onesi_browser = webdriver.Firefox()
        self.addCleanup(lambda: quit_if_possible(onesi_browser))
        self.browser = onesi_browser
...
and `onesi_browser` is also assigned to `self.browser` at the point at which the 
...
[role="sourcecode"]
.functional_tests/test_sharing.py (ch24l008)
====
[source,python]
----
        # She notices a "Share this list" option
