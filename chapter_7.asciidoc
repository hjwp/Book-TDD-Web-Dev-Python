...
with that now.  Change its value to a temp. folder (I'm using
'/tmp/teststatic', but if you're on Windows, for example, you'll want to 
use something else.
...
And if we look in '/tmp/teststatic', we'll find all our CSS files:
...
This is useful to collect all the static files from several apps, and then put 
them into a single directory, which we can then tell the web server to server
directly.

We'll find out all about that in the next chapter!

For now we can blow away our little experiment (warning: use this command with
caution, as it will blow away any changes you haven't committed)
