...
brittle with respects to cosmetic changes in our `<input>` tag.

Django does provide the `html=True` argument to `assertContains()`,
but even that is a little too fragile to do what we want.

In tests like this one, we really don't want to be distracted
by extraneous attributes like the CSS class or the HTML id.
...
Let's switch to using a proper HTML 
