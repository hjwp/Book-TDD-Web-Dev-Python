...
[...]
AssertionError: False is not true : Couldn't find '<input name="item_text"' in the following response
[...]
----
...
In tests like this one, we really don't want to be distracted
by extraneous attributes the HTML id or the exact placeholder text.
...
<3> The `[form] =` is worth a mention.
    ...
    we're effectively asserting that there is exactly one element on the right; if there isn't, we'll get an error.
