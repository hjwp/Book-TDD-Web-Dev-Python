...
from a command line (usually with the command *`python3`*), and to know how to
edit a Python file and run it.  Again, have a look at the three books I
...
NOTE: If you already have Python 2 installed, and you're worried that
    ...
    coexist peacefully on the same system, and they each store their packages
    in totally different locations.  You just need to make sure that you have
    one command to launch Python 3 (`python3`), and another to launch Python 2
...
A virtualenv with Python 3, Django 1.8 and Selenium 2 in it:: 
...
3. When you install Python 3, make sure you tick the option that says 
*"Add Python 3.5 to PATH"* as in <<add-python-to-path>>, so that you can run
...
Let's create a virtualenv called "superlists" that has Python 3 installed:
...
We'll install Django 1.8 and Selenium 2:

[subs=quotes]
----
(superlists) $ *pip install "django<1.9" "selenium<3"*
Collecting django<1.9
  Using cached Django-1.8.16-py2.py3-none-any.whl
Collecting selenium<3
  Using cached selenium-2.53.6-py2.py3-none-any.whl
Installing collected packages: django, selenium
Successfully installed django-1.8.16 selenium-2.53.6
----
...
# this means you haven't added virtualenvwrapper to your .bashrc.  Fix:
*echo "source virtualenvwrapper.sh" >> ~/.bashrc*
*source ~/.bashrc*
