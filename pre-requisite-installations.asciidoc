...
command prompt and just run `git`, `python3` or `pip3` from any folder.
...
a terminal and just run `python3` or `pip3` from anywhere.  Once you've
installed Django (see below), you should also be able to just run
`django-admin.py` from any folder too.

TODO: update these instructions for Python 3.4. Add a note re Git?
