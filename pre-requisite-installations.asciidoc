...
3.6 isn't available directly from your package manager: On Ubuntu I recommend
the https://launchpad.net/~fkrull/+archive/ubuntu/deadsnakes[Deasnakes PPA];
alternatively, compiling Python :.6 from source is actually surprisingly easy.
...
you're working in a folder called *python-tdd-book*, but you can name your work
folder whatever you like.  Stick to the name "venv" for the virtualenv though.
...
On Windows, we first need to install the "virtualenv" module to make
virtualenvs compatible with Git-Bash:
...
The `py` executable is a shortcut for different Python versions on Windows)
...
you'll see `(superlists)` in parentheses, in your prompt.  But you can
...
venv/bin/activate` (on Mac/Linux) and `source venv/Scripts/activate` on
Windows. The command to deactivate is just `deactivate`.
...
/home/myusername/python-tdd-book/venv/bin/python
...
(superlists)$
(superlists)$ *which python*
/C/Users/harry/python-tdd-book/venv/Scripts/python
(superlists)$ *deactivate*
$
$ *which python*
...
install Django 1.11 and the latest Selenium, Selenium 3.  Remember to make sure
your virtualenv is active first!
...
quick `source venv/Scripts/activate` or `source venv/bin/activate` is probably
what you need to get it working again.
...
In the world of web programming it's much less necessary, so **I recommend you
do not use Anaconda for this book**.
