...
On Windows, we use the `py` executable, which is a shortcut for different
Python versions:
...
you'll see `(venv)` in parentheses, in your prompt.  But you can
...
----
(venv) $ *pip install "django<1.12" "selenium<4"*
...
bash: venv/Scripts/activate.bat: line 4: syntax error near unexpected token `(
