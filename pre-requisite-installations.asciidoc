...
* For macOS or Linux, one convenient place to put it is __&#x7e;/.local/bin__footnote:[
  as long as '&#x7e;/.local/bin' is on your path. If it's not, it's worth adding it,
  because that's where `pip install --user` puts things.  We won't use `pip
  install --user` in this book but it does come in useful in everyday life...]
...
'.bashrc':footnote:['.bashrc' is an initialization file for Bash that lives in
your home folder. It gets run every time you start Bash]
