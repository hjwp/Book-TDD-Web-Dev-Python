...
I also had to assign a "named" regex capture group:
...
And I was able to get the view working using multiple inheritance, and an
override of the `get_form` method:
...
just drop me a line at obeythetestinggoat@gmail.com.
...
Python would have it. I mean... `SingleObjectMixin`?  What? Still, I guess some
...
Class-Based Generic Views can do anything::
    It might not always be readable, or clear what's going on, but you can 
    do just about anything with class-based views.

Single-assertion unit tests help refactoring::
    With each unit test providing individual guidance on what works and what
    doesn't, it's much easier to change the implementation of our views so
    drastically.
