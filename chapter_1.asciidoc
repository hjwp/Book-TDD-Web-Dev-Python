...
[NOTE]
----
Did `vi` just pop up for you and you had no idea what to do with it? Well, you
have two choices. You can either learn some minimal vi commands (press `i` to 
go into insert mode, type your text, presc `Esc` to go back to normal mode,
then write the file and quit with `:wq<Enter>`). You'll then have joined the
great fraternity of people who know with this ancient, revered text editor.

Or you can point-blank refuse to countenance such a ridiculous throwback to the
