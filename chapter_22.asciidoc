...
even though the actual effect of the code would be the same.
...
the implementation much more closely, so it's less likely to be a problem. We'll
come back to this.
...
the term Hexagonal Architecture] to describe this pattern, since I believe
he was the original person to talk about the "Ports and Adapters" pattern.
