...
Also, not all browsers fully implement HTML5, so some users are still likely to
see our custom error message. And if or when we come to letting users access
our data via an API (see <<appendix_rest_api>>), then our validation messages
will come back into use.
