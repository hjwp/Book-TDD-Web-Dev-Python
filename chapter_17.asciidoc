...
NOTE: This chapter not yet been updated for the new edition.  You won't be
    able to follow along with some of it if you have the code from the new
    version of chapters 16 + 17
...
[subs="specialcharacters,quotes"]
----
elspeth@server: *sudo service gunicorn-superlists-staging.ottg.eu restart*
----
...
Traceback (most recent call last):
  File "/workspace/superlists/functional_tests/test_my_lists.py", line 34, in
test_logged_in_users_lists_are_saved_as_my_lists
    self.assert_logged_in(email)
  File "/workspace/superlists/functional_tests/base.py", line 42, in
assert_logged_in
    self.browser.find_element_by_link_text('Log out')
[...]
...
FAIL: test_can_get_email_link_to_log_in (functional_tests.test_login.LoginTest)
 ---------------------------------------------------------------------
Traceback (most recent call last):
  File "/workspace/superlists/functional_tests/test_login.py", line 22, in
...
pre-authenticated session.  There's some kind of bug.



When I ran into this bug, I did consider just going back and fixing it in the
previous chapter, and pretending it never happened, but I think leaving it
illustrates the point of running tests against a staging environment.  It would
have been pretty embarrassing if we'd deployed this bug straight to our live
site.
...
[subs="specialcharacters,quotes"]
----
elspeth@server: $ *service gunicorn-superlists-staging.ottg.eu restart*
...
at\n5.5.1  https://support.google.com/mail/answer/14257 [...]
...
TODO: finish rewriting this chapter

* add EMAIL_PASSWORD to gunicorn service config
* ? discuss difference with secret_key
* still won't work, implement testing of email in FT
* discuss pros+cons of same.


WARNING: the rest of this chapter still references the old login system.
