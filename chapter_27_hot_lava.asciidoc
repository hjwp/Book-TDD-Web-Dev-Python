...
involving the database in every single test starts to carry an unacceptable cost—in terms of execution speed.((("database testing", "functional and integration tests getting slower"))) Casey's company, for example, was struggling with test suites that took several hours.
...
that do not depend on those, um, dependencies.((("business logic", "decoupling from its dependencies")))((("dependencies", "decoupling business logic from")))
...
and then using that interface at test time to swap out fake versions of your real dependencies.((("architectures of applications", "hexagonal/clean/onion architecture")))((("adapters, ports and")))((("object-oriented architecture", "ports and adapters")))((("Hexagonal Architecture patterrn")))((("Clean Architecture pattern")))((("Onion Architecture pattern")))
...
functional tests, or even (gasp!) not at all (if it's kept minimal enough).((("Functional Core, Imperative Shell Architecture pattern")))((("architectures of applications", "Functional Core, Imperative Shell")))((("core application code", "functional core, imperative shell")))
...
Kent Beck tells ((("observability and monitoring")))((("production", "testing in")))a story about his first few weeks at Facebook,
