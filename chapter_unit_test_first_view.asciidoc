...
((("units tests", "vs. functional tests", secondary-sortas="functional tests")))((("functional tests (FTs)", "vs. unit tests", secondary-sortas="unit tests")))As with so many of the labels we put on things, the line between unit tests and
...
Our tests are telling us that we need a URL mapping. Django uses a file called
...
((("", startref="DJFunit03")))That was quite a chapter! Why not try typing `git log`, possibly using the
...
*******************************************************************************
((("Django framework", "commands and concepts")))Running the Django dev server::
