...
test_root_url_resolves_to_home_page_view
  ...
  File ".../django/urls/resolvers.py", line 520, in resolve
...
WARNING: If your _urls.py_ looks different or if it mentions a function called
    ...
    Django.  This book is written for Django 2.1.  Take another look at
    the "<<pre-requisites>>" section and get the right version before you
    go any further.

A `path` entry starts with a string that defines which URLs it
...
The first example entry has the path `''`, which means
