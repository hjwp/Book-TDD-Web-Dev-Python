...
((("databse migrations", id="dbmig30")))Django-migrations and its predecessor South have been around for ages,
so it's not usually necessary to test database migrations.  But it just
so happens that we're introducing a dangerous type of migration, i.e., one
...
pointers here to get started with.((("", startref="dbmig30")))
