...
* ((("cheat sheet", "project setup")))((("Django framework", "set up", "project creation")))start with a 'User Story' and map it to a first 'functional test'.
...
* ((("cheat sheet", "moving beyond dev-only testing")))Start system testing early. Ensure your components work together: web server,
...
* ((("cheat sheet", "testing best practices")))((("testing best practices", "general", "testing only one thing")))Each test should test one thing.

* ((("testing best practices", "general", "using one test file per application code source file")))One test file per application code source file.

* ((("testing best practices", "general", "using placeholder tests for every function and class")))Consider at least a placeholder test for every function and class,
  no matter how simple.

* "Don't test constants".

* ((("testing best practices", "general", "test behavior rather than implementaion")))Try to test behaviour rather than implementation.

* ((("testing best practices", "general", "think beyond the charmed path through code")))Try to think beyond the charmed path through the code, and think
...
* ((("testing best practices", "Selenium/functional tests", "using explicit waits")))Use explicit rather than implicit waits, and the interaction/wait pattern.

* ((("testing best practices", "Selenium/functional tests", "avoiding duplication")))Avoid duplication of test code--helper methods in base class, or Page
  pattern are one way to go.

* ((("testing best practices", "Selenium/functional tests", "avoiding double-test functionality")))Avoid double-testing functionality. If you have a test that covers a 
...
* ((("testing best practices", "Selenium/functional tests", "using BDD tools")))Look into BDD tools as another way of structuring your FTs.
...
((("cheat sheet", "isolated vs. integrated tests")))Be reminded of the reason we write tests in the first place:
