...
Time for test for the positive case where authenticate should return a user object.
...
To get back to a working state, let's define a temporary placeholder for `get_user`:
...
our User model too.  Oh well.  My amazing single-field user model is ruined. I
