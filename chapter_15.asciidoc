...
The system was invented by someone who was annoyed at having to create
new passwords for so many websites, and found himself just using random,
...
https://TODO-insert-link-here[read all about it here]
...
packages, like https://TODO-another-url[django-social-auth] and
https://TODO-another-other-URL[python-social-auth], but both of them looked
overcomplicated for this stage (and I was quietly relieved since I was rather
looking forward to writing my own code for this!)

from Dan's talk and the example code on the, but by the end I had something
which just about works.  I'll take you on a tour, and then we'll go through and
"de-spike" the implementation.  
...
- when someone wants to log in, we generate a unique secret token for them,
    store it in the database linked to their email, and email it to them
...
def send_login_email(request):
    ...
    url = 'http://localhost/accounts/login/{token}/'.format(token=token.uid))
    send_mail(
...
[role="sourcecode"]
.accounts/templates/login_email_sent.html
[source,html]
----
{% extends 'base.html' %}
...
we need to tell Django our email server address.  I'm just
using my gmail account for now.  You can use any email provider you like,
as long as they support SMTP.
...
email address with a unique id.  The Python standard library's `uuid` module is
useful here:
...
TIP: the `default` argument accepts a value or a function. If you
pass it a function, then it'll call it each time we create a new
instance, which means the value can change for each instance. That's
why I didn't put `uuid.uuid4()`.
...
email address.  We'll neaten this up when we de-spike.
...
Some URLs for our three views:

[role="sourcecode"]
.accounts/urls.py (ch15l008)
[source,python]
----
...
A quick `makemigrations` to make the new user model real:
