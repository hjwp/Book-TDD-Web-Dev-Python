...
((("user authentication",id="ix_userauth", range="startofrange")))((("authentication systems", id="ix_authsyst", range="startofrange")))Our beautiful lists site has been live for a few days, and our users are
...
module is ready and waiting for you. It's nice and straightforward, and I'll leave
it to you to discover on your own.)

((("authentication", "mocking", see="mocks/mocking")))((("mocks/mocking", "in JavaScript")))In this chapter, we're going to get pretty deep into a testing
...
((("Mozilla Persona")))((("Persona (Mozilla)")))((("authentication", "Mozilla Persona")))But which federated authentication system to use?  Oauth?  Openid?  "Login with
...
((("JavaScript", "spiking with", seealso="spiking", id="ix_javaspiking", range="startofrange")))((("spiking", "with JavaScript", id="ix_spikingjava", range="startofrange")))((("authentication", "spiking", id="ix_authspiking", range="startofrange")))((("spiking", id="ix_spiking", range="startofrange")))((("exploratory coding", see="spiking")))Before I wrote this chapter all I'd seen of Persona was a talk at PyCon by Dan
...
((("Django-BrowserID")))The first thing I did was take a look at an existing  Django-Persona
integration called
...
((("spiking", "frontend and JavaScript code")))Let's start with the frontend.  I was able to cut and paste code from the
...
((("spiking", "browser-ID protocol")))Persona will now pop up its authentication dialog box if users
...
((("spiking", "server-side authentication", id="ix_spikingssa", range="startofrange")))((("authentication","customising", id="ix_authcust", range="startofrange")))((("Django", "authentication in", id="ix_Djangoauth", range="startofrange")))Next we prep an app for our accounts stuff:
...
(((range="endofrange", startref="ix_spikingssa")))(((range="endofrange", startref="ix_authcust")))(((range="endofrange", startref="ix_Djangoauth")))Almost there. We switch on the auth backend and our new accounts app in
'settings.py':
...
((("debugging", "Ajax")))That's pretty much it! Along the way, I had to fight pretty hard, including
...
overcome a bugfootnote:[http://stackoverflow.com/questions/16983547/django-problems-with-id-in-custom-usermodel/18458659#18458659]
that later turned out to be irrelevant.((("Ajax")))((("debugging")))
...
((("spiking", "de-spiking", id="ix_despiking", range="startofrange")))((("functional tests/testing (FT)", "for de-spiking", sortas="despiking")))((("JavaScript", "de-spiking in", id="ix_javadespiking", range="startofrange")))De-spiking means rewriting your prototype code using TDD.  We now have enough
...
((("MockMyID")))((("Persona Test User")))

[[firefox-debug-persona]]
.Using the Debug toolbar to find locators
image::images/twdp_1503.png["The Firefox debug toolbar open on the Persona screen"]
((("functional tests/testing (FT)", "for evaluating third-party systems", sortas="evaluating")))((("payment systems", "testing for")))
...
((("Selenium", "explicit waits")))((("waits")))((("explicit waits")))Here's the first of the two "wait" helper functions:
...
(((range="endofrange", startref="ix_authspiking")))(((range="endofrange", startref="ix_spiking")))(((range="endofrange", startref="ix_despiking")))(((range="endofrange", startref=""ix_javadespiking")))
...
((("static folder, site-wide")))A bit of housekeeping first: create a site-wide static files directory inside
...
((("mocks/mocking", "in JavaScript", id="ix_mockingjava", range="startofrange")))We want our login link's on-click to be bound to a function provided by
...
((("namespacing")))((("mocks/mocking", "namespacing")))In the context of 'base.html', `navigator` is just an object in the global
...
into a local variable by passing it into an "initialize"footnote:[UK-English speakers may bristle at that incorrect spelling of the
...
programming model that's tied into a global scope, and this sort of namespacing/naming convention helps to keep things under control.  Lots of JavaScript 
libraries might want to call a function `initialize`, but very few will call
it `Superlists.Accounts.initialize`!footnote:[The new shiny in the JavaScript world for avoiding namespacing
...
((("mocks/mocking", "initialize function test", id="ix_mockinginitialize", range="startofrange")))The `initialize` function itself we 'will' test.  Copy the lists
...
((("JavaScript", "debug console")))((("debugging", "in JavaScript")))In Firefox and I believe Chrome also, you can use the shortcut Ctrl-Shift-I
...
((("assertion messages")))((("QUnit")))NOTE: Assertion messages (the third argument to `equal`), in QUnit, are
...
(((range="endofrange", startref="ix_mockinginitialize")))A quick check on the FT ... doesn't get any further unfortunately.  To see
...
((("watch function")))Decoding that, the `watch` function needs to know a couple of things from 
the global scope:  
...
((("mocking", "sinon.js")))((("mocking library")))((("sinon.js")))"Rolling your own" mocks is possible as we've seen, and JavaScript actually
...
((("mocking", "checking call arguments")))((("call arguments")))We're not calling the `watch` function correctly yet--it needs to know
...
((("Fake XMLHttpRequest")))((("QUnit","setup and teardown")))((("sinon.js")))Next we need to check the `onlogin` callback, which is called when Persona
...
.Aside on Ajax
*******************************************************************************
((("Ajax")))If you've never used Ajax before, here is a very brief overview.  You may find
...
((("functional tests/testing (FT),"and asynchronous code", sortas="asynchronous", id="ix_FTasynch", range="startofrange")))((("mocking", "callbacks", id="ix_mockingcallbacks", range="startofrange")))((("asynchronous code", id="ix_asynchcode", range="startofrange")))((("sinon.js")))This is what JavaScript's all about folks!  Thankfully, sinon.js really does
...
(((range="endofrange", startref="ix_mockingjava")))(((range="endofrange", startref="ix_FTasynch")))(((range="endofrange", startref="ix_mockingcallbacks")))(((range="endofrange", startref="ix_asynchcode")))

.On Spiking and Mocking with JavaScript
*******************************************************************************
Spiking::
    ((("spiking")))Exploratory coding to find out about a new API, or to explore the
...
Mocking::
    ((("mocking")))We use mocking in unit tests when we have an external dependency that we
