...
<3> Rather than using a "real" email address and having to click through 
    ...
    http://www.mockmyid.com[MockMyID] is one, you can also check out
    http://wwww.personatestuser.org[Persona Test User].
...
Testing should be part of how you evaluate 3rd party systems.  When you integrate
with an external service, you're going to have to think through how you're going
to work with it in your functional tests.
...
party service.  In the case of this integration with Persona, we could have used
a "real" email address; when I first wrote this chapter, I actually had an FT that
clicked through to Yahoo.com, and logged in with a throwaway account I'd created.
The problem is that it made the FT totally reliant on particular details of Yahoo's
email login screens, which can change at any time.
...
you're going to want to make sure they're tested thoroughly... But you don't want
to be putting actual transactions on real credit cards through, every time you
run an FT!  So most providers will provide a "test" version of their payments API.
These vary in quality (naming no names), so make sure you investigate them
thoroughly.
...
TIP: `implicitly_wait` is unreliable. Use "wait-for" patterns in your FT
    whenever you need to check for asynchronous interactions on your pages.
...
//TODO: restore dofirst. these mvs are ugly
