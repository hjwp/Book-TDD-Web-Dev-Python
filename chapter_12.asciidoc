...
((("duplicates, eliminating", id="ix_duplicateselim", range="startofrange")))((("functional tests/testing (FT)", "for duplicate items", sortas="duplicateitems", id="ix_FTduplicates", range="startofrange")))We add a second test method to `ItemValidationTest`:
...
exists.']}
----
((("functional tests/testing (FT)", "and developer stupidity", sortas="developer")))
...
((("QuerySet", id="ix_quersetordering", range="startofrange")))When we run the tests they reveal an unexpected failure:
...
Now we're down to two failures, and the ordering test has a more readable failure pass:[<phrase role='keep-together'>message:</phrase>]
...
((("model-layer validation", "at views level", sortas="views")))Let's try running our FT, just to see where we are:
...
((("functional tests/testing (FT)", "in views", sortas="views")))((("views, what to test in")))
