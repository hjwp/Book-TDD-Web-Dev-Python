...
TODO: Some users have reported they don't see this unexpected fail. Test on
various platforms, amend if necessary.
...
error, adjust its error message, and then pass it back up to the form.
...
Next -- let's find places where we should be using our new form class. There's
two places in our view tests where we check the form, let's let's adjust them
both:
