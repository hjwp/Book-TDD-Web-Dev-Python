...
NOTE: This chapter goes into more intricate details of Django's form
validation, and you can consider it optional.  If you're still learning Django,
there's good stuff in here.  If you want to skip ahead, that's OK too. Make
sure you take a quick look at the aside on developer stupidity, and the recap
on testing views at the end.
...
We'll want to do a bit of testing at the model layer.  But first, let's 
tidy up one of the tests in there.


Rewriting the model tests 
^^^^^^^^^^^^^^^^^^^^^^^^^

I wrote `test_saving_and_retrieving_items` in a very verbose style, to
introduce the Django ORM, but in fact we can get the same coverage from a
couple of much shorter tests:

[role="sourcecode"]
.lists/tests/test_models.py (ch12l002)
[source,python]
----
...
[role="sourcecode"]
.lists/tests/test_models.py (ch12l003)
[source,python]
----
...
I'd commit that on its own...


Rewriting the overly verbose model test
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
//TODO: simplify first model test at this point?? if not in 10.
...
Traceback (most recent call last):
  File "/workspace/superlists/lists/tests/test_models.py", line 63, in
...
The next task is to handle the validation error in the view. 

Before we do that, a quick aside, for the curious. Do you remember I mentioned
earlier that some data integrity errors 'are' picked up on save?  Try doing a
`.save` instead of a `.full_clean` in our duplicates test:
...
//TODO: consider moving this into previous
