...
This test uses the raw view function, and to manually construct an
...
But it won't actually work, because we haven't been down to the model layer
yet.  Our lists don't know how to save a list owner: 
...
At this stage, I'm sorely tempted to just leave this test failing, and just 
jump down to the models layer (and I should thank to Gary Bernhardt, one 
of the Testing Goat's great prophets, for pulling me up and making me do it
"properly").

In a simple example like this, we can probably get away with leaving the tests
for the outer layer failing while we wait to implement the inner layer that
they depend on.  But imagine if our app was more complex, and there were
several more layers between the outside and the inside.  Imagine leaving
three or four or five layers of tests, all failing while we wait to get to 
the bottom layer to implement our critical feature.  While tests are failing,
we're not sure that layer really works, on its own terms, or not.  We have
to wait until we get to the bottom layer.

Instead, rigorous outside-in TDD suggests that you 'isolate' each layer from
the next one.  

We can achieve this using a bit of mocking:
...
it hard work, it's likely that your tests are trying to tell you something. 7
lines of setup (3 lines for the mock user, and 4 more lines for the request 
object) is way too many.

What this test is telling us is that our view may be doing too much work,
dealing with creating a form, creating a new list object 'and' deciding whether
or not to save an owner for the list.

We'll come back to this in the next chapter, when we explore the topic of
test isolation in more detail.
...
[subs="specialcharacters,quotes"]
----
$ *git commit -am"slightly more isolated tests at views layer"*
$ *git tag needs_moar_isolation*
----
...
Our outside-in design has driven out two requirements for the models layer:
we want to be able to assign an owner to a list using the attribute
`.owner`, and we want to be able to access the lists owner by a user with 
