...
Assuming you have a working "my lists" page from the last section, we'll
now try and deploy our site to staging, and run the tests there.  As we
do so, we'll discover an unexpected bug.  Once we've wiped our brow and
finished thanking the Goat that our staging environment helped us catch
it before we deployed it to real users, we'll implement a fix.  

We'll also learn about how to adapt our FT's fixture-creation code to
manage the database on the test server.


Deploying to staging, and finding our bug
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

let's see how we get on with deploying our code to the server...
...
Now we can re-run our FTs, and get to the next bug: 
