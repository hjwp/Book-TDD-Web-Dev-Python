...
<3> We want to use `list.name` to print out the "name" of the list, which is
    currently specified as the text of its first element.((("programming by wishful thinking")))
...
((("mocks/mocking", "in Outside-In TDD", sortas="outside-in")))In order to get this test passing, as it's written now, we have to move
