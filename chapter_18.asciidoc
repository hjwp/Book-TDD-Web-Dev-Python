...
* Set it to poll every hour (check out the help text here --
...
You can test this first by deliberately breaking one of the tests. Then,
commit, push, and when we re-run the build on Jenkins, we see this:

----
screenshotting to /var/lib/jenkins/jobs/Superlists chapter 18
(CI)/workspace/functional_tests/screendumps/LoginTest.test_login_with_persona-window0-2014-01-22T17.45.12.png
dumping page HTML to /var/lib/jenkins/jobs/Superlists chapter 18
(CI)/workspace/functional_tests/screendumps/LoginTest.test_login_with_persona-window0-2014-01-22T17.45.12.html
----
...
or, if you feel like it, you can use another package manager called
"Bower":
...
I won't explain bower any further, but you should look into it if you find
yourself using more than one or two front-end packages (like Bootstrap). In
this case we just used it as a glorified download tool, but it can be much
more.

In any case, you should end up with this:
...
TODO: discuss the fact that Persona sometimes gets stuck on Captchas.  Switch
to mockmyid or similar?
