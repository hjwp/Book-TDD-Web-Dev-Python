...
((("functional tests (FTs)", "with multiple users", secondary-sortas="multiple users", id="FTmultiple25")))((("functional tests (FTs)", "structuring test code", id="FTstructure25")))Are jokes about how "everything has to be social now" slightly old hat?
...
((("Page pattern", "FT with multiple user")))Let's get started--we'll need two users for this FT:
...
((("Page pattern", "reducing duplication with", id="POPduplic25")))((("duplication, eliminating", id="dup25")))Before we go any further, I want to show an alternative method for reducing
...
((("", startref="POPduplic25")))((("", startref="dup25")))The next step would be to pursue the FT refactor through our other tests. I'm
...
((("Page pattern", "extending FTs to second users")))Let's spec out just a little more detail of what we want our sharing user
...
((("Page pattern", "practical exercise")))There's nothing that cements learning like taking the training wheels off,
...
The Page pattern::
    ((("Page pattern", "benefits of")))Moving helper methods into a base `FunctionalTest` class can become 
