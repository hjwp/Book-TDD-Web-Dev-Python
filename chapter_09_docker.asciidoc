...
=== Debugging a Container That Doesn't Seem to Work at All
...
currently looking, on 'localhost:8888'.
...
[subs="specialcharacters,macros"]
----
$ pass:quotes[*TEST_SERVER=localhost:8888:8000 ./manage.py test functional_tests \
    --failfast*]
...
We're pretty sure the server is running and listening on port 8000,
but we can't get to it from the outside.
