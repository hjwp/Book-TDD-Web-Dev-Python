...
=== Aside: Selenium and Firefox Upgrades
...
and showed me a strange popup window, <<firefox_upgrade_popup>>
...
It turns out that Firefox had automatically downloaded a new version,
and when Selenium was trying to run it, it wanted to install the latest version
of its "driver" plugin.

In order to resolve the issue, I had to close the Selenium browser window,
go back to my main browser window and tell it to install the upgrade and restart itself,
and then try again.
