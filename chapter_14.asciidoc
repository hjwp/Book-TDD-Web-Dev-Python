...
1 assertions of 1 passed, 0 failed.
    ...
    initialize binds sign in button to navigator.id.request (0, 1, 1)Rerun
----
...
TODO: messages in jquery tests are actually susccess messages.  rewrite all of
these!
...
----
    initialize binds sign in button to navigator.id.request (1, 1, 2)
        called request before click triggered
        Expected: 	false
        Result: 	true
----
...
A good start!  Let's try pulling it into our template:
...
test("initialize calls navigator.id.watch", function () {
    ...
    var mockWatchFunction = function (params) { //<2>
        equal(params.loggedInUser, loggedInUser, 'wrong user'); //<3>
        equal(params.onlogin, Superlists.Accounts.onLogin, 'wrong onlogin function'); //<4>
        equal(params.onlogout, Superlists.Accounts.onLogout, 'wrong onlogout function'); //<4>
        watchFunctionCalled = true;
    };
    var mockNavigator = {
        id: {
            watch: mockWatchFunction //<2>
        }
    };

    Superlists.Accounts.initialize(mockNavigator, loggedInUser, urls); //<1>
...
initialize calls navigator.id.watch (1, 0, 1)Rerun1 ms
    Died on test #1
...
And we're off!  Our mock function is telling us that we're not calling it
correctly, it wants us to pass in the user inside the params object.  Let's
do that:
...
been set, so it is `undefined`... but `Superlists.Accounts.onLogin` doesn't
exist either, so it is also `undefined`.  And `undefined == undefined`.
Thankfully. Nothing is certain in JavaScript, as TDD supremo Gary Bernhardt 
once hilariously demonstrated in a talk called
...
validated it.  And to do that, we'll need at least the `onLogin` function.
