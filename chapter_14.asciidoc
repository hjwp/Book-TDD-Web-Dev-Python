...
one for pass:[<span class="keep-together">example:</span>]
...
1. errors should be hidden on keypress (1, 0, 1)
    1. failed
        Expected: false
        Result: true
2. errors not be hidden unless there is a keypress (1)
----
...
When does our code in lists.js run, and when does each of our tests run?  And
...
* 'lists.js' loads first.  So our event listener should be attached to the
  input element
...
lists.js sees and attaches the event listener to will be replaced with a new
...
than just relying on the code in lists.js running whenever it is loaded by
a `<script>` tag, we can use a fairly common pattern, which is to define an
"initialize" function, and we can call that when we want to in our tests (and
later in real life).
...
And in our tests, we call initialize with each test:
...
And now we should see our tests pass, and our debug output should make
more sense:
...
In addition, our "initialize" function name is too generic.  Let's 
give ourselves a namespace that's unlikely to be used by anyone else:
...
NOTE: Are you wondering about the `/*global` comment? I'm using a tool called
    `jslint`, which is a syntax-checker for Javascript that's integrated into
    my editor. The comment tells it what global variables are expected--it's
    not important to the code, so don't worry about it, but I would recommend
    taking a look at Javascript linters like `jslint` or `jshint` when you get
    a moment. They can be very useful for avoiding JavaScript "gotchas".
    ((("JavaScript", "linters")))
    ((("global variables")))
