...
There are some alternatives out there—http://nose.readthedocs.org/en/latest/[nose] is popular, and I've personally
found http://pytest.org/[pytest] to be very impressive.  But there is
a clear default option, and it's just fine.footnote:[Admittedly once you start looking for Python BDD tools, things are a
...
* At the moment our test only checks that the JavaScript works on one page.
  ...
  added it to 'home.html' the tests would still pass.  It's a judgment 
