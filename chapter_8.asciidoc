...
[...]
OK
$ ls ../virtualenv/lib/python2.7/site-packages/
django                       setuptools-0.6c11-py2.7.egg
Django-1.5.1-py2.7.egg-info  pip-1.3.1-py2.7.egg             setuptools.pth
easy-install.pth             
----
...
haven't set up the database. Let's do that now.
...
Hooray!  We're reassured that some of the piping works, but we really can't be
...
Now if you reload gunicorn and take a look at the site, you'll find the CSS is
all broken.  In fact, if we run the functional tests, you'll see they tell us
...
for you, Gunicorn doesn't.  Now is the time to get Nginx to server static files
for us.
...
And now we tell Nginx to start serving those files directly:
...
`proxy_set_header` is needed to make sure gunicorn and Django know what domain
it's running on.  We need that for the `ALLOWED_HOSTS` security feature, which 
we're about to switch on.  First, we re-run the functional test again, to make
sure things still pass.
...
exec ../virtualenv/bin/gunicorn --bind /tmp/superlists-staging.ottg.eu.socket superlists.wsgi:application
----
...
'/etc/init/gunicorn-superlists-staging.ottg.euf'

Now we can start gunicorn with

    sudo service gunicorn-superlists-staging.ottg.eu start
...
////
TOOD: add ALLOWED_HOSTS to version control?
...
def _update_settings(source_folder, site_name):
    ...
    append(settings_path, 'ALLOWED_HOSTS = "%s"' % (site_name,)) #<8>

def _update_virtualenv(source_folder):
    virtualenv_folder = path.join(source_folder, '../virtualenv')
    if not exists(path.join(virtualenv_folder, 'bin', 'pip')): #<9>
...
def _update_static_files(source_folder):
    run('cd %s && ../virtualenv/bin/python manage.py collectstatic --noinput' % ( # <10>
        source_folder,
    ))
...
command-line, eg 'superlist.ottg.eu'.
...
<8> The fabric `sed` command does a string substitution in a file, here it's changing
DEBUG from True to False.  `append` just adds a  line to the end of a file.
<9> We look inside the virtualenv folder for the `pip` executable as a way of
checking whether it already exists.
<10> We use the virtualenv version of python whenever we need to run a Django 
...
for an existing site as well as for a new one.  If you like words with Latin roots, 
you might describe it as idempotent, which means it does nothing if run twice...
...
[superlists-staging.ottg.eu] out: Requirement already satisfied (use --upgrade to upgrade): Django==1.5.1 in ./sites/superlists-staging.ottg.eu/virtualenv/lib/python2.7/site-packages (from -r /home/harry/sites/superlists-staging.ottg.eu/source/requirements.txt (line 1))
[superlists-staging.ottg.eu] out: Requirement already satisfied (use --upgrade to upgrade): argparse==1.2.1 in /usr/lib/python2.7 (from -r /home/harry/sites/superlists-staging.ottg.eu/source/requirements.txt (line 2))
[superlists-staging.ottg.eu] out: Requirement already satisfied (use --upgrade to upgrade): wsgiref==0.1.2 in /usr/lib/python2.7 (from -r /home/harry/sites/superlists-staging.ottg.eu/source/requirements.txt (line 3))
[superlists-staging.ottg.eu] out: Cleaning up...
[superlists-staging.ottg.eu] out: 
[superlists-staging.ottg.eu] run: cd /home/harry/sites/superlists-staging.ottg.eu/source && ../virtualenv/bin/python manage.py collectstatic --noinput
[superlists-staging.ottg.eu] out: 
[superlists-staging.ottg.eu] out: 0 static files copied, 9 unmodified.
[superlists-staging.ottg.eu] out: 
[superlists-staging.ottg.eu] run: cd /home/harry/sites/superlists-staging.ottg.eu/source && ../virtualenv/bin/python manage.py syncdb --noinput
...
////
TODO: add output from sed and append
////
...
brrrp, brrrp>' noises like Mother in Alien).  Anyway, we look through it,
...
packages we need.  `collectstatic` also notices that the static files are
all already there, and finally the `syncdb` completes without a hitch.

So, let's try using it for our live site!

[subs="specialcharacters,macros"]
----
$ pass:quotes[*fab deploy:host=superlist.ottg.eu*]
[superlist.ottg.eu] Executing task 'deploy'
[superlist.ottg.eu] run: mkdir -p /home/harry/sites/superlist.ottg.eu
[superlist.ottg.eu] Login password for 'harry':  <1>
[superlist.ottg.eu] run: mkdir -p /home/harry/sites/superlist.ottg.eu/database
[superlist.ottg.eu] run: mkdir -p /home/harry/sites/superlist.ottg.eu/static
[superlist.ottg.eu] run: mkdir -p /home/harry/sites/superlist.ottg.eu/virtualenv
[superlist.ottg.eu] run: mkdir -p /home/harry/sites/superlist.ottg.eu/source
[superlist.ottg.eu] run: git clone https://github.com/hjwp/book-example.git /home/harry/sites/superlist.ottg.eu/source
[superlist.ottg.eu] out: Cloning into '/home/harry/sites/superlist.ottg.eu/source'...
[superlist.ottg.eu] out: remote: Counting objects: 461, done.
[superlist.ottg.eu] out: remote: Compressing objects: 100% (262/262), done.
[superlist.ottg.eu] out: Receiving objects: 100% (461/461), 137.86 KiB, done.
[superlist.ottg.eu] out: Resolving deltas: 100% (208/208), done.
[superlist.ottg.eu] out: 
[superlist.ottg.eu] run: virtualenv /home/harry/sites/superlist.ottg.eu/source/../virtualenv
[superlist.ottg.eu] out: New python executable in /home/harry/sites/superlist.ottg.eu/source/../virtualenv/bin/python
[superlist.ottg.eu] out: Installing setuptools............done.
[superlist.ottg.eu] out: Installing pip...............done.
[superlist.ottg.eu] out: 
[superlist.ottg.eu] run: /home/harry/sites/superlist.ottg.eu/source/../virtualenv/bin/pip install -r /home/harry/sites/superlist.ottg.eu/source/requirements.txt
[superlist.ottg.eu] out: Downloading/unpacking Django==1.5.1 (from -r /home/harry/sites/superlist.ottg.eu/source/requirements.txt (line 1))
[superlist.ottg.eu] out:   Downloading Django-1.5.1.tar.gz (8.0MB): 8.0MB downloaded
[superlist.ottg.eu] out:   Running setup.py egg_info for package Django
[...]
[superlist.ottg.eu] out: Successfully installed Django
[superlist.ottg.eu] out: Cleaning up...
[superlist.ottg.eu] out: 
[superlist.ottg.eu] out: Copying '/home/harry/sites/superlist.ottg.eu/source/lists/static/base.css'
[...]
[superlist.ottg.eu] out: Copying '/home/harry/sites/superlist.ottg.eu/source/lists/static/bootstrap/img/glyphicons-halflings-white.png'
[superlist.ottg.eu] out: 
[superlist.ottg.eu] out: 9 static files copied.
[superlist.ottg.eu] out: 
[superlist.ottg.eu] run: cd /home/harry/sites/superlist.ottg.eu/source && ../virtualenv/bin/python manage.py syncdb --noinput
[superlist.ottg.eu] out: Creating tables ...
[superlist.ottg.eu] out: Creating table auth_permission
[...]
[superlist.ottg.eu] out: Installing indexes ...
[superlist.ottg.eu] out: Installed 0 object(s) from 0 fixture(s)
[superlist.ottg.eu] out: 

Done.
Disconnecting from superlist.ottg.eu... done.
----
...
----
sed "s/SITENAME/superlist.ottg.eu/g" deploy_tools/nginx.template.conf | sudo tee /etc/nginx/sites-enabled/superlist.ottg.eu
----
...
----
$ sudo ln -s ../sites-available/superlist.ottg.eu /etc/nginx/sites-enabled/superlist.ottg.eu
----

Now we write the upstart script:

----
sed "s/SITENAME/superlist.ottg.eu/g" deploy_tools/nginx.template.conf | sudo tee /etc/init/gunicorn-superlist.ottg.eu.conf
----
...
service gunicorn-superlist.ottg.eu start
----
...
you wouldn't have to worry about this...  Also, if I go for domain sockets
from the word go, then I can't start off using the dev server, so we also have
to bite the virtualenv/gunicorn bullets at the same time.
- am getting annoyed at using the full domain name in every single config file.
Particularly for the upstart scripts, it sucks because they won't tab
complete...
