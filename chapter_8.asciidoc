...
----
(virtualenv)$ sudo ln -s ../sites-available/$SITENAME /etc/nginx/sites-enabled/$SITENAME
----
...
And now to test it:
    ...
    (virtualenv)$ sudo service nginx reload
    (virtualenv)$ python manage.py runserver
...
debug telling us as much as the tests went through, or if you tried it manually.
...
restart the dev server with `manage.py runserver`

Hooray!  Well, we're reassured that some of the piping works, but we really
can't be using the Django dev. server in production.  We also can't be relying
...
Now if take a look at the site, you'll find the CSS is all broken:
...
I saved this file into '/etc/init' on the server, calling it
'/etc/init/gunicorn-superlists-staging.ottg.eu'
...
main function is called `deploy`, that's the one we'll invoke from the
...
<8> We `reset --hard` to blow away any current changes on the server's tree,
directory.  In both the `clone` and `git pull`, it's assumed that we want to
deploy whatever the head of the 'master' branch is, so if you're using a 
different branch this won't work.
...
'settings'py. Then `pip install -r requirements.txt`, completes happily, noting
...
----
sudo service nginx reload
sudo service gunicorn-superlists.ottg.eu start
----
