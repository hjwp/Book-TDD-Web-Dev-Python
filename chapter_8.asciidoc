...
Installing Apache + pals
^^^^^^^^^^^^^^^^^^^^^^^^

All the cool kids are using things like Nginx, uWSGI or Gunicorn these days, 
but we'll use rusty old Apache because it's been around for ages, which means
it's easy to install on any platform and there's lots of help out there for it

////
TODO: switch to Arch
I chose Centos as my distro, because it's one I'm not familiar with. I know
some of you reading may have never done this stuff before, so I wanted to give
myself at least some of that feeling of the unknown, to sympathise.

Installing apache was a matter of running `yum install httpd`.  On Ubuntu
or Debian, it's `apt-get install apache2`
////

Installing Apache on my server was a matter of doing an `apt-get`, and I could
then see the default Apache "Hello World" screen:

    apt-get install apache2

.Apache - It works!
image::images/apache_it_works.png[The default Apache "It Works!" page]

We'll need three other bits of software on the server: mod_wsgi, Git and Pip:
...
We'll also install a system-wide version of Django.  Later, we'll see how
to use a Virtualenv so that different sites can run on different versions
...
Next, I don't want to be messing about with IP addresses all the time, so I
decided to point a couple of sub-domains I own towards my new server.  One
domain for "live", and one for staging

.Domain setup
image::images/domain_setup.png[Registrar control screens for two domains]


Once these were setup, I checked they worked by visiting both domains, and
seeing the same Apache "it works" screens.

The final step is to get a copy of the staging site up and running, just
to check whether we can get Apache's Python web app serving working. It's 
called *mod_wsgi*, and there are some official instructions on how to get 
it set up with Django here:

https://docs.djangoproject.com/en/1.5/howto/deployment/wsgi/modwsgi/

We're almost done with "provisioning" and can start thinking about
the "deployment" part. Let's see if we can get the staging site set up,
manually, on the server, and take some notes so that we know how to automate
the process for future upgrades, and for deploying the live site.
...
We start by checking out a copy of our repo in a temp directory.

    cd /tmp/
    # you should replace the next line with the URL to your own repo
    git clone https://github.com/hjwp/book-example.git repo-checkout
...
code at. Finally, the virtualenv gets its own subfolder too.
...
in bash:
   ...
   cp -r repo-checkout /home/harry/sites/$SITENAME/source
