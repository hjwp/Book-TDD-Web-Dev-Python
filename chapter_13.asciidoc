...
second time.((("Django", "migrations")))((("migrations")))
...
((("deployment", "staging")))We start with the staging server:
...
((("deployment", "to live", sortas="live")))Assuming all is well, we then run our deploy against live:
...
((("migrations")))Because our migrations introduce a new integrity constraint, you may find
...
((("Git tags")))The last thing to do is to tag the release in our VCS--it's important that
