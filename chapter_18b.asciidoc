...
If you don't see a pass, make sure that your views code in 'views.py' is exactly
as I've shown it, using `List()`, not `List.objects.create`.  
...
----
    ...
    def test_redirects_to_form_returned_object_if_form_valid(
        self, mock_redirect, mockItemForm  #<2>
    ):
        mock_form = mockItemForm.return_value
...
<2> `patch` decorators are applied innermost first, so the new mock is injected
    to our method as before the `mockItemForm`.
...
TIP: When using assert methods on mocks, like `assert_called_once_with`,
    ...
    `asssert_called_once_with`, try it!)
...
So on the one hand, the isolated test is harder to read.  It takes a good deal
...
@@ -23,19 +23,10 @@ class ItemFormTest(TestCase):
...
[source,diff]
----
@@ -99,18 +90,8 @@ class NewListFormTest(TestCase):
...
to refactor the view function?  I'd like to think I probably would have, but
it's hard to be sure.  But writing isolated tests does make you very aware of
where the complexities in your code lie.
...
[subs="specialcharacters,quotes"]
----
$ *git commit -am"add list owners via forms. more isolated tests"
$ *git checkout master*
