...
your commit message in.  Mine looked like <<first_git_commit>>.footnote:[Did vi pop up and you had no idea what to do?  Or did you see a message about account identity and `git config --global user.username`? Go and take another look at the preface, there are some brief instructions.]
...
have excellent documentation. Alternatively, you can wait until Chapter 8 when we'll be using one for deployment.
