[[chapter-1]]
Getting Django Set Up Using a pass:[<phrase role='keep-together'>Functional Test</phrase>]
------------------------------------------------------------------------------------------
...
(((Testing Goat)))((("Goat", see="Testing Goat")))The Testing Goat is the unofficial mascot of TDD in the Python testing
...
(((Django)))We'll proceed with nice small steps; we're going to use 'Django', which is
a popular Python web framework, to build our app. 

(((Selenium)))The first thing we want to do is check that we've got Django installed, and
...
(((functional test (FT))))That's our first 'functional test' (FT); I'll talk more about what I mean by
...
((("Django", "running"))) Since you've definitely read <<pre-requisites>> by now, you've
...
((('superlists')))That will create a folder called 'superlists', and a set of files and
...
((('manage')))You'll also have noticed 'manage.py'. That's Django's Swiss Army knife, and
...
((("Git", "repository setup", id="ix_"Git", "repository_setup", range="startofrange)))((("version control system (VCS)", id="ix_version_control_system_VCS", range="startofrange")))((("VCS (version control system)", id="ix_VCS_version_control_system", range="startofrange")))There's one last thing to do before we finish the chapter: start to commit our
...
((('superlists')))Let's start by moving 'functional_tests.py' into the 'superlists' folder, and
...
(((commit)))Looking good, we're ready to do our first commit!
...
well-deserved pat on the back before moving on to <<chapter-2>>.(((range="endofrange, startref=ix_"Git", "repository setup")))(((range="endofrange, startref="version_control_system_VCS")))(((range="endofrange, startref=ix_"VCS_version control system")))
