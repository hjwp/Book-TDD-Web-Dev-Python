...
you should see. At that point, you'll have to do some debugging. This will be
an excellent, character-building experience. Be tenacious, read the error
message carefully, and you'll get to the bottom of it. It's probably just a
missing comma, or trailing-slash, or maybe a missing "s" in one of the selenium
find methods.  But, as <<lpthw,Zed Shaw put it so well>>, this kind of
debugging is an absolutely vital part of learning, so do stick it out.  And
drop me an email (or try the
https://groups.google.com/forum/#!forum/obey-the-testing-goat-book[Google
Group]) if you get really stuck.
