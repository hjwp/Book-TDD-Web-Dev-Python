...
image::images/wrong_order_list.png["A screenshot of the todo list from the FT, with Make Tea appearing above Purchase Milk"]
...
We can fix it by converting the QuerySet to a list,footnote:[You could also check out `assertSequenceEqual` from `unittest`, and
