...
((("database testing", "HTML POST requests", "creating", id="DBIpostcreate05")))((("forms", "wiring up to send POST request", id="ix_form")))
...
and revisit the smoke test of our home page template content.((("frontends", "testing contract between backend and")))((("backends", "testing contract between frontend and")))
...
Everything else is cosmetic. So that's what we test for in our unit tests.((("forms", "wiring up to send POST request", startref="ix_form")))
...
Conveniently, we've already got a ((("sleep", "debugging using time.sleep")))sleep just before the error occurs;
...
Ross Anderson's https://www.cl.cam.ac.uk/~rja14/book.html[_Security Engineering_].
...
*******************************************************************************
((("unit tests", "versus integration tests", secondary-sortas="integration tests")))
((("integration tests", "versus unit tests", secondary-sortas="unit tests")))
