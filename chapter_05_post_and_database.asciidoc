...
----
AssertionError: False is not true : Couldn't find '<input name="text"' in the following response
b'<html>\n  <head>\n    <title>To-Do lists</title>\n  </head>\n  <body>\n
<h1>Your To-Do list</h1>\n    <form method="POST">\n      <input
...
And even if we were feeling like we could get away with out,
...
.An Alternative Testing Strategy Using a More Black Box Approach
*******************************************************************************
...
"test commands via public side-effects", in her talk
...
so that the database stays an implementation detail,
you prove to yourself that todo items are persisted,
by sending more than one:

[role="sourcecode skipme"]
.lists/tests/test_views.py (ch05l025-1)
====
[source,python]
----
