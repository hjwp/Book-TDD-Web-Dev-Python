...
- And finally, 'automation, automation, automation'.  By using an automated
  ...
  like live as possible.footnote:[What I'm calling a "staging" server, some people would call a
...
<1> `setUpClass` is a similar method to `setUp`, also provided by `unittest`,
    which is used to do test setup for the whole class. This means it only
...
We're going to need a couple of domain names at this point in the book--they can both be subdomains of a single domain.  I'm going to use
...
registered a domain before, just pick any old registrar and buy a cheap one--it should only cost you $5 or so, and you can even find free ones.
...
Django-1.7-py3.3.egg-info  pip-1.4.1-py3.3.egg-info  setuptools-0.9.8-py3.3.egg-info
