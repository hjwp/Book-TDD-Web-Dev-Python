...
-- it should only cost you $5 or so. I don't want to endorse any particular
service, but there are even free ones out there. And I promise seeing your site
on a "real" web site will be a thrill :-)
...
Internet, and that you can SSH into it.  I recommend Ubuntu as a distro,
...
useradd -m elspeth # add user and create home folder for them
usermod -a -G sudo elspeth # add elspeth to the sudoers group
passwd elspeth # set user's password
su - elspeth # switch-user to being elspeth!
----

I recommend learning up how to use SSH key authentication rather than
passwords for SSH.  It's a matter of taking the public key from your
own PC, and appending it to '~/.ssh/authorized_keys' in the user
account on the server. You probably went through a similar procedure if 
you signed up for Bitbucket or Github.
...
Your `pip-3.3` command may be called `pip3`, depending on what version of
Ubuntu you have.  Just make sure you find it, and not the Python 2 one.
...
- git tag the commit deployed as a release.
