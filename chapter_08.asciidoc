...
"Welcome to nginx" page at this point:
...
screens looked a bit like this:
...
To confirm  this works, we can re-run our functional tests and see that their
...
folder for a non-root user, in my case it would be at '/home/elspeth' (this is
...
* When using Git-Bash, do not try and run 'activate.bat' -- it is written for
...
[subs="specialcharacters,quotes"]
----
(virtualenv)$ *pip install django==1.6.1*
[...]
...
TIP: Notice you don't have to use the `activate` to use the virtualenv.
...
beginner-friendly nano. Note you'll also need to use `sudo` because the file
...
[subs="specialcharacters,quotes"]
----
elspeth@server:$ *../virtualenv/bin/python3 manage.py syncdb --noinput*
Creating tables ...
[...]
...
//TODO: this could be a good place to pause and make a new chapter.
...
setuid elspeth <3>
chdir ~/sites/superlists-staging.ottg.eu/source  <4>
...
<3> `setuid` changes to run as the "elspeth" user
...
setuid elspeth
chdir ~/sites/superlists-staging.ottg.eu/source
...
TDD as a way of confirming a stable deployment
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

During this chapter, we practised a bit of server administration, which is
always fun (for certain values of fun).  The point I was trying to highlight
was that having a functional test suite can take some of the uncertainty out
of the process.  Whenever we make a change to our server configuration, we
can re-run the test suite, and be confident that everything works as well as
it did before.  It allows us to experiment with our setup with less fear.
