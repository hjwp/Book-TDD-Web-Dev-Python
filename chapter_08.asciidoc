...
((("staging site", "functional tests (FT) for")))((("functional tests/testing (FT)", "for staging sites", sortas="stagingsites")))Let's adapt our functional tests slightly so that it can be run against
...
((("servers", seealso="staging server")))((("deployment testing", "manual provisioning for hosting", id="ix_deptestprov", range="startofrange")))((("hosting, manual provisioning", id="ix_hostingmp", range="startofrange")))((("servers, manual provisioning for hosting", id="ix_servermp", range="startofrange")))((("provisioning", id="ix_provisioning", range="startofrange")))We can separate out "deployment" into two tasks:
...
((("Nginx")))((("static files", "Nginx and")))First we run `collectstatic` to copy all the static files to a folder where 
Nginx can find them:
