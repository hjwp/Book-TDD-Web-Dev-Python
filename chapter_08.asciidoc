...
<5> `ExecStart` is the actual process to execute.  We use the `\ ` line
...
Now we can start Gunicorn with the `service` command:

[role="server-commands"]
[subs="specialcharacters,quotes"]
----
elspeth@server:$ sudo service gunicorn-superlists-staging.ottg.eu start
----

(you should find this command responds to tab-completion, including of the
service name, by the way)
