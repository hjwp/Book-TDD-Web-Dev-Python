...
((("Django", "and virtualenvs")))We could install it at this point, but that would leave us with a problem:  if
...
the first few chapters as simple as possible.(((range="endofrange", startref="ix_virtualenvs")))


Simple Nginx Configuration
^^^^^^^^^^^^^^^^^^^^^^^^^^

((("Nginx", id="ix_Nginx", range="startofrange")))Next we create an Nginx config file to tell it to send requests for our staging
...
complex? You can't check everything. The tests can.(((range="endofrange", startref="ix_Nginx")))
