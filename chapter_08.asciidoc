...
pieces of software we need at the system level: Python, Git, pip, and
virtualenv.

[role="server-commands"]
[subs="specialcharacters,quotes"]
----
elspeth@server:$ *sudo apt-get install git python3*
elspeth@server:$ *sudo apt-get install python3-pip python3-venv virtualenvwrapper*
----
...
Creating a Virtualenv using requirements.txt
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
[subs="specialcharacters,quotes"]
----
$ *echo "Django==1.8.16" > requirements.txt*
...
elspeth@server:$ *mkvirtualenv --python=python3 superlists-staging.ottg.eu*
elspeth@server:(superlists-staging.ottg.eu)$ *pip install -r requirements.txt*
Downloading/unpacking Django==1.8.16 (from -r requirements.txt (line 1))
[...]
...
I named the virtualenv after the site name too, dots are allowed in
virtualenv names, and, as a convention, it guarantees the virtualenv
name will be unique, and it's easy to tell which one is which!
...
[subs="specialcharacters,quotes"]
----
elspeth@server:$ *pip install gunicorn*
----
...
[subs="specialcharacters,quotes"]
----
elspeth@server:$ *gunicorn superlists.wsgi:application*
...
[subs="specialcharacters,quotes"]
----
elspeth@server:$ *python manage.py collectstatic --noinput*
...
ExecStart=/home/elspeth/.virtualenvs/superlists-staging.ottg.eu/bin/gunicorn \ <4>
    --bind unix:/tmp/superlists-staging.ottg.eu.socket \  <5>
...
<4> `ExecStart` is the actual process to execute.  Notice that we don't use
    `workon` to activate our virtualenv, instead we specify the full path
    to the version of Gunicorn that's in our virtualenv. That automatically
    gives it access to everything else that's in the virtualenv.

<5> I'm using the +\+ line continuation character to split the full command over
    multiple lines, for readability, but it could all go on one line.
...
4. `manage.py migrate` for database
5. `collectstatic` for static files
6. Set DEBUG = False and ALLOWED_HOSTS in 'settings.py'
...
ExecStart=/home/elspeth/.virtualenvs/SITENAME/bin/gunicorn \
