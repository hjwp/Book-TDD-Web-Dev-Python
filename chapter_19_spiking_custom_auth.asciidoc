...
<1> `django.contrib.auth.get_user_model()` is a function that returns the
    user model class that is currently active in this project.
    This will allow us to also test that we've configured our custom model
    to be the active one for the project,
    and that Django is happy that it's a valid user model.

<2> And here's our test.  Like our model, it's nice and minimal!
Just for fun, let's try importing the "standard" Django user model,
and seeing what the tests think about it:
...
<1> django.contrib.auth.get_user_model() is a function that returns
    the currently-configured user model.
    At this stage, that's Django's default one.
...
That gives us an expected failure; Django picks up its "normal" default user model,
which has fields we don't want:
...
That gives us an error because Django won't all models,
...
There's a built-in function in `django.contrib.auth` called `get_user_model()`
...
In fact, it's so unhappy that it wont even run the tests:
