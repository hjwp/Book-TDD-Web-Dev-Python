...
((("database input", "HTML POST requests", "creating", id="DBIpostcreate05")))((("POST requests", "creating", id="POSTcreate05")))At the end of the last chapter, the tests were telling us we weren't able to
...
*******************************************************************************
((("Cross-Site Request Forgery")))((("security issues", "Cross-Site Request Forgery")))If you've never heard of a 'Cross-Site Request Forgery' exploit, why not look
...
((("database input", "HTML POST requests", "processing")))((("POST requests", "processing")))Because we haven't specified an `action=` attribute in the form, it is
...
<1> If you've not seen this syntax before, it's the new Python "f-string"
...
Unexpected failure::
    ((("Test-Driven Development (TDD)", "concepts", "unexpected failure")))((("unexpected failure")))When a test fails in a way we weren't expecting.  This either means that
