...
// CSANAD:  I think it would be a good idea to suggest using OWASP as a reference for IT
//          security: https://owasp.org/www-community/attacks/csrf
...
one thing at a time.]
// CSANAD:  "doing things in tiny little really does" seems to be a word missing from here,
//          "...steps" maybe? However, this footnote seems to be unrelated to the
// sentence it's referred to: "Ok that gets our unit tests passing, but it's not really
// what we want."
...
// CSANAD:  I think the last sentence is a little too complex like this. I would
//          split it into two sentences like:
//  Once again we change our unit test for saving a POST request. From now on,
//  instead of rendering a response with the item in it, it should redirect
...
// CSANAD:  I can't find where we used in-line print, maybe it's been removed?
