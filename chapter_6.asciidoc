...
Traceback (most recent call last):
 ...
 line 63, in test_list_view_displays_all_items
    response = client.get('/lists/the-only-list-in-the-world/')<2>
  File "/usr/local/lib/python2.7/dist-packages/django/test/client.py",
 line 439, in get
  [...]
  File "/usr/local/lib/python2.7/dist-packages/django/views/defaults.py",
 line 20, in page_not_found <3>
    t = loader.get_template(template_name) # You need to create a 404.html template.<4>
 ...
 line 138, in find_template
    raise TemplateDoesNotExist(name)
TemplateDoesNotExist: 404.html<1>
...
This is a good example to use as a little aside on how to read tracebacks -- in
TDD we do spend a lot of time looking at tracebacks (although I seem to
remember looking at a lot of tracebacks before my TDD days, but that may be
because I used to write very buggy code).

The first things to look at are the top and the bottom.  The bottom should tell
you something about what the actual problem is: <1> in this case, Django is
complaining that it cannot find a template called '404.html'.

Then, it's often useful to look up to the top, to try and find the line from
your test code that started everything off: <2> in this case, it was when
we tried to get the '/the-only-list-in-the-world/' URL, which we may remember
doesn't actually exist yet.

For more information, you can start to look between the two.  You might look
for a line of your own code, application code this time, which has caused the
problem.  Or, as in this case, it may be external library code, which can
tell you something more about what happened.

Here you can see that the Django Test Client goes through much more of the
Django stack than our other unit tests do -- it's gone through the URL
resolution part of the stack (which we used to call separately via `resolve`),
and come to the conclusion that the URL we asked for doesn't exist,  when it
gets to the view in 'defaults.py' called `page_not_found` <3>.  Following on
the rest of the traceback, you can see it then tries to load a 404 template,
but it can't find one and so raises a `TemplateDoesNotExist` error.  There's
even one of Django's classic helpful comments telling us that we need to create
one at <4>
...
$ *git status*  # should show 2 new files in green
...
That now passes.  We're looking good. Can we remove the old code from
`home_page`?
...
ID for now.  We change `ListViewTest`, make it use the
...
and the captured text will get passed to the view as its first argument. 
But our view doesn't expect an argument yet! Sure enough, this causes
problems:
...
Let's take a look at this test then, since it's whining.  Hm, it 
looks like it hasn't been adjusted to the new world of Lists and Items.
In fact, this brings to mind the fact that we actually need to treat
the creation of 'new' lists differently from the addition of new items
to 'existing' lists.  
...
That still gives us the invalid literal error. Let's take a look at the
view itself, and change it so it redirects to a valid place:
...
TODO - adapt to new helper function
...
up and... oh.  The template needs to know what list it's rendering,
as well as what items.  Let's adjust our unit test `ListViewTest`, and
add an extra check at the end:
