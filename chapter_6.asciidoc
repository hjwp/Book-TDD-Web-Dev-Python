...
Let's translate our to-do list into our functional test.  As soon as Edith
...
There are 4 items on our to-do list. We'll start on the second one, which is
to do with adjusting the URLs for our site to be more in line with our design,
since that's what the FT is currently stuck on.  It's complaining that the URL
you get taken to after starting a list isn't a list URL.  So let's start by
...
That gets the tests back to passing.  Now let's do a little more tidying
...
This is because the view is actually giving a 404 (we can check
