...
<3> Wait actually--also, we're hardcoding the creation of a user every time,
...
but we don't need to test that every colour works for every engine type (ie, we don't need all 9 tests).
...
[role="skipme"]
----
_create_engine(engine_type) -> 3 tests
_assemble_car(engine_type, car_type) -> 9 tests
_paint_car(colour, body_type) -> 3 tests

build_car(car_type, engine_type, colour) -> ? tests
----
...
----
build_car(car_type, engine_type, colour) -> ? tests
  engine = _create_engine(engine_type)   # only depends on one thing
  naked_car = _assemble_car(engine, car_type)  # depends on engine.type and car_type
  finished_car = _paint_car(naked_car, colour)  # depends on naked_car.type and colour
  return finished_car
...
>>>>>>> b019a496 (Wip on alternative flow thru the remove duplication bit)
...
In this section we'll look at a different possible rationale for using mocks.
Here we don't have any side effects we're worried about,
but there are still potentially some reasons you might want to use a mock here.
...
because we've decided to use a _particular_ interface to our authentication backend,
which means we're more justified in using mocks to verify that we use that specific interface.
