...
def login(request):
    if user := auth.authenticate(uid=request.GET.get("token"))  # <1>
        ...
        messages.error(request, "Invalid login link, please request a new one")  # <2>
...
Let's see how it might look.  We'll work by adding the mocky tests first,
get them all passing, and then deleting the nonmocky ones
(`test_logs_in_if_given_valid_token()` and `test_shows_login_error_if_token_invalid()`):
...
[role="sourcecode"]
.src/accounts/views.py (ch19l048)
====
[source,python]
----
def login(request):
...
That gets us an unexpected failure!
...
E.......
======================================================================
ERROR: test_logs_in_if_given_valid_token  <2>
  File "...goat-book/src/accounts/tests/test_views.py", line 71, in
test_logs_in_if_given_valid_token
    self.client.get(f"/accounts/login?token={token.uid}")  <3>
[...]
  File "...goat-book/src/accounts/views.py", line 34, in login
    user = User.objects.create(email="edith@example.com")  <4>
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File
"...goat-book/.venv/lib/python3.12/site-packages/django/db/models/manager.py",
line 87, in manager_method
    return getattr(self.get_queryset(), name)(*args, **kwargs)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
[...]
django.db.utils.IntegrityError: UNIQUE constraint failed: accounts_user.email  <1>
----

We apply our usual traceback-reading heuristics.
<1> The error is about unique constraints, that's not immediately obvious.
<2> Aha, this isn't the test we just wrote, it's another test,
    one of our non-mocky ones
<3> Here's the line in our tests that caused the error, no surprise here
<4> And here's the line in our own code.
    If we look back in our view, we can see it's _after_ we call `auth.authenticate()`,
    in our pre-existing code where we manually create a user.

Aha!  `authenticate()` itself already creates a user for us,
so we can't create a second one with the same email address.
Thanks, database constraint!


Fixing that step-by-step, I went through this stage,
where we get the user from `authenticate()` (which by itself doesn't fix the error)
and then remove the `User.objecs.create()` line (which does):
...
[source,diff]
----
@@ -29,9 +29,8 @@ def send_login_email(request):
...
And now we've been through Red and Green, let's do a little refactor!
This is one of those places where it's actually nice to use the Walrus operator:


[role="sourcecode"]
.src/accounts/views.py (ch19l050)
====
[source,python]
----
def login(request):
...
We've now got a mixture of mocky and non-mocky tests.
Since we decided to go all in on mocks here, let's keep going,
until our tests at the views layer are properly isolated from the auth backend.
...
[role="sourcecode"]
.src/accounts/tests/test_views.py (ch19l049)
====
[source,python]
----
@mock.patch("accounts.views.auth")  # <1>
...
// TODO: add a failure messages.message?  will help ppl with debugging login failures
