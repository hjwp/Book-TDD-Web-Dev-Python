...
// JAN: It the previous version of the function_tests.py, we have if __name__ == "__main__". It's not there anymore despite the facet that code snippet seems "copy&paste" ready. This might be a bit confusing. I'd suggest to either add to or explicitly mark the code that was update (e.g., with ## UPDATE START and ## UPDATE END comments)
...
//         chapter_unit_test_first_view.asciidoc#urlspy we said "We don’t need two
...
// CSANAD:  ...and rename our unit test? I was a bit confused here where the previous
//          test_home_page_returns_correct_html has gone and where test_uses_home_template
//          came from. It seems like this is what happened, we renamed the test.
...
// JAN: I must say I have very mixed feelings about this way of testing. I get your point about testing plain HTML strings - makes complete sense. But I don't think using self.assertTemplateUsed(response, "home.html") makes it much better. We go from HTML implementation details (self.assertContains(response, "<title>To-Do lists</title>")) to Django implementation details (self.assertTemplateUsed(response, "home.html")). This test will pass whenever I'm using "home.html" template. It may be empty or complete garbage - this test will pass as long as the naming is correct. On the other hand, if I change the name of template, I'll have to update the test. Logic and ouput might stay completely the same. (I wouldn't be surprised if something like this would happen also when changing template engine). In other words, this test is not resistant to refactoring while also not protecting much against regression. Therefore, I'd consider it as low quality test - it won't protect me much while posibly failing when things still work exactly as expected. I know that there's not much breathing room there to actually change much. Nevertheless, I think it's worth bringing this up. (I derived this approach for test quality evaluation from this book: https://www.manning.com/books/unit-testing?query=%20Vladimir%20Khorikov)
