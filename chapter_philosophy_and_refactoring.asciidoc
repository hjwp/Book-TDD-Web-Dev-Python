...
functions and pass:[<span class="keep-together">constants</span>].  
...
Instead, Django gives us a tool called the https://docs.djangoproject.com/en/1.11/topics/testing/tools/#the-test-client[Django Test Client], which has
...
Each chapter has its own branch which is named after its short name, like so:
