...
* TODO: update to new ansible config where env vars are passed directly

[role="sourcecode"]
.infra/env.jinja (ch23l011)
====
[source,python]
----
...
// DAVID: This feels like a logical leap. As a reader, I'm unsure whether I was supposed
// to have known this, or if you're just saying 'trust me, I'll save you some time by telling you why'.
...
// DAVID: Would be good to mention security at this point. We're rolling our
// own way to authenticate, so we should tread carefully. Maybe point out that
// in order to use this approach, we need ssh access to the server, so it's
// decently protected.
...
====
//14
// DAVID: My immediate reaction is that this shouldn't be enabled in production.
// Is it worth calling that out at the time?
// Perhaps at least only add it if an environment variable is present?
...
// DAVID: Confused here what you mean by local versus staging server.
// Is the Docker image the local server or staging server? Maybe should call out that there are
// three different test server contexts we're supporting...
...
----
====
// DAVID: container_commands doesn't exist yet, maybe worth pointing this out
// so readers don't think they've made a mistake?
...
// DAVID: Should this final one be 'Against Docker remotely'?
// Also, there are rendering problems on the last one.
...
// DAVID: I got
// Exception: OCI runtime exec failed: exec failed: unable to start container process:
// exec: "/venv/bin/python": stat /venv/bin/python: no such file or directory: unknown
// Changing it to /usr/local/bin/python worked. (Maybe I just set this up on an earlier version of the book.)
