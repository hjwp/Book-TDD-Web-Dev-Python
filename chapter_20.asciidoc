...
----
//TODO: rename Oni to Francis, as per ch. 6?
...
is only allocated half-way through a test, so you don't have to spend time in
...
Let's see how we would Page objects for the home and lists pages. Here's one
for the home page:
...
[role="sourcecode"]
.functional_tests/home_and_list_pages.py (ch20l006)
[source,python]
----
[...]
...
[role="sourcecode"]
.functional_tests/test_sharing.py (ch20l007)
[source,python]
----
...
[role="sourcecode"]
.functional_tests/test_sharing.py (ch20l008)
[source,python]
----
        # She notices a "Share this list" option
...
[role="sourcecode"]
.functional_tests/home_and_list_pages.py (ch20l009)
[source,python]
----
...
[role="sourcecode"]
.functional_tests/test_sharing.py (ch20l010)
[source,python]
----
    [...]
    list_page.share_list_with('oniciferous@email.com')
...
[role="sourcecode"]
.functional_tests/home_and_list_pages.py (ch20l011)
[source,python]
----
class HomePage(object):
...
[role="sourcecode"]
.functional_tests/test_sharing.py (ch20l012)
[source,python]
----
...
[role="sourcecode"]
.functional_tests/home_and_list_pages.py (ch20l013)
[source,python]
----
...
It's long past time to run the FT and check all of this works!
...
* So before we can get this test to pass, we have to move down to the models
...
* Then back up to the template level, on the "my lists" page we'll want a
  ...
  page, we want also want to show who the list is shared with, as well as
...
* You might find that spinning up the site with `runserver` will help you 
  iron out any bugs, as well as for fine-tuning the layout and aesthetics.
...
[[list-sharing-example]]
.Sharing lists
image::images/list_sharing_example.png["Screenshot of list sharing UI"]
