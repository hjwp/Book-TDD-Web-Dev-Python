...
so that we can save it somehow and display it back to them later.  For now,
we'll use a standard HTML form and POST request.  A little boring, but also
...
different URLs for adding new lists, new items, 3 new view functions. I could
jump ahead and write tests for all of them at once. But I stopped myself, and
decided to present things in a more iterative fashion instead: I would
take one step at a time, solve the immediate problem needed to get the 
functional tests a little further, and only that problem, even if I
...
needing to call `.save()`. And that gets the test passing.
