...
It's a demonstration of how TDD can supports an iterative style of development
...
ends up in the rendered template. That gives us our expected fail:
...
This is the whole point of having tests.  Now, perhaps you saw it coming, but
imagine if you'd been having a bad day or weren't paying attention: our tests
have just saved us from accidentally breaking our application, and, because
we're using TDD, we found out immediately.  We didn't have to wait for a QA
team, or click through our site manually, and we can get on with fixing it
...
an implemention where we ``cheat'' to one we're happy with?
...
Remember, I'm not saying you should ignore glaring problems like this in ``real
life''. When we spot problems in advance, it's usually a good idea to deal with
them straight away, if we can.  But we don't always spot problems in advance,
and sometimes the solution isn't obvious.  What I'm demonstrating here is the 
way that TDD can help guide you to the right answer, even when you 'don't'
