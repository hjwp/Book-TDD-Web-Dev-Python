...
with named `new_item_text`, whose value is the expected item text from our
POST request.
...
so we are allowed to re-write our view, and tell it too to pass the POST
...
I've codes a very naive solution and you can probably spot a very obvious
