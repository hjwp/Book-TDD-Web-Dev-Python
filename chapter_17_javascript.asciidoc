...
https://developer.mozilla.org/en-US/docs/Web/JavaScript#for_complete_beginners[MDN]
...
The test might be happy, but our solution is a little unsatisfactory.((("DevTools (developer tools)", "editing HTML in")))
...
((("Jasmine")))((("unittest module", "how testing works with")))
...
Let's see how that looks.((("web browsers", "running Jasmine spec runner test")))
...
Is the Jasmine standalone browser test runner unconventional?
I think it probably is, to be honest.((("Jasmine", "standalone browser test runner")))
...
NPM, the massive _node_modules_ folder—and a very steep learning curve.((("frameworks", "JavaScript")))
...
Read up on `checkVisibility()` method in the https://developer.mozilla.org/en-US/docs/Web/API/Element/checkVisibility]MDN documentation].]
...
(From now on, I'll show the Jasmine outputs as text, as in <<first-jasmine-output>>,
...
If you really can't use Firefox, you might be able to find some solutions on https://stackoverflow.com/questions/20748630/load-local-javascript-file-in-chrome-for-testing[Stack Overflow].]
]
...
to remove some duplication.((("CSS (Cascading Style Sheets)", "removing hardcoded selectors")))((("Red/Green/Refactor", "removing hardcoded selectors")))((("selectors (CSS)", "removing hardcoded selectors")))  We'll start with that,
...
and invoke our initialize function on our real pages. Let's use another `<script>` tag to include our _lists.js_, and strip down the the inline JavaScript to just calling `initialize()` with the right selectors:
...
still left around our input box. Wait a minute!  We forgot one of the key things we learned in our spike!
...
https://developer.mozilla.org/en-US/docs/Web/API/Window/load_event[The MDN documentation] on this is good, as usual.
