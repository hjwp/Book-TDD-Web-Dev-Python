...
((("forms", "simple", id="ix_formsimple", range="startofrange")))((("Django", "forms in", see="forms")))
...
maybe even to a non-Django module that represents your business logic? ((("code smell")))
...
((("forms", "experimenting with")))Let's do a little experimenting with forms by using a unit test.  My plan is to
...
((("forms", "customising form field input")))((("widgets")))That gives us a fail which justifies some real coding.  How can we customise
...
http://django-floppyforms.readthedocs.org/[django-floppyforms]
for some help.
...
((("forms", "ModelForm")))((("forms", "autogeneration")))((("ModelForm")))What's next?  We want our form to reuse the validation code that we've already
...
((("Meta")))In `Meta` we specify which model the form is for, and which fields we want it
to use.
...
((("widgets")))It's lost our placeholder and CSS class. But you can also see that it's using
...
((("forms", "validation testing and customising")))Now let's see if the `ModelForm` has picked up the same validation rules which we
...
changing `error_messages`, another `Meta` pass:[<phrase role='keep-together'>variable:</phrase>]
...
((("forms", "using in views", id="ix_formsinviews", range="startofrange")))I had originally thought to extend this form to capture uniqueness validation
...
((("GET requests")))Let's start in our unit tests for the home view. We'll replace the old-style
...
((("forms", "find and replace in")))We'll need to fix this, and it's going to involve a big find and replace.
...
((("POST requests", id="ix_POSTreq", range="startofrange")))Now we want to adjust the unit tests for the `new_list` view, especially the
...
And while we're at it, we'll use a constant instead of a hardcoded string
...
((("GET requests")))This view handles both GET and POST requests.  Let's start with checking 
...
((("helper functions/methods")))Next we want to use the form errors in the second view.
...
((("forms", "save methods")))There are a couple more things we can do to make our views even simpler.  I've
...
next chapter.(((range="endofrange", startref="ix_formsimple")))
((("forms", "thin views")))((("thin views")))((("forms", "tips for")))
.Tips
*******************************************************************************
Thin views::
    ...
    +
    +
    Another possible place would be a custom method on the model class.  And--once the complexity of the app demands it--out of Django-specific files
...
Each test should test one thing::
    ...
    and it's worth rewriting it as several tests. Helper functions can keep
