...
28, in test_cannot_add_empty_list_items
    error = self.browser.find_element_by_css_selector('.has-error')
[...]
selenium.common.exceptions.NoSuchElementException: Message: Unable to locate
element: .has-error
...
Not quite, but they did get a little further.  Checking 'line 28', we can
...
Hm, that's strange, they're not even getting past our first check for
the error.  Can you guess why?


Another explicit wait for Selenium
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

It's another case of Selenium trying to look for elements before a new page
has loaded.  We need an explicit wait in between our attempt to submit
an empty element, and looking for the error message.

NOTE: Depending on the speed of your PC, and how the Selenium project evolves,
    your test may not have failed at exactly this point.  But replacing an
    implict/lucky wait with an explicit wait is always a good idea in any case.


Here's the simplest possible wait:
