...
Displaying errors is a job our form should be doing!  Before we make any more
changes though, let's do a commit:
...
* If there's a validation error, we should render the home template
...
And while we're at it, we'll use a constant instead of a hard-coded string
...
Traceback (most recent call last):
  File "/workspace/superlists/lists/tests/test_views.py", line 54, in
...
inputs.  They would have been left lying around our database.  We can add
an extra assert for that, just to make sure we don't make that mistake
again:


[role="sourcecode"]
.lists/tests/test_views.py (ch11l025)
[source,python]
----
...
At this point the FT will still fail, because we're not yet using the form
...
//TODO: slight inconsistency in tests. this one munges form and error...
