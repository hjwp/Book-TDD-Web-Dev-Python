...
[role="sourcecode"]
.lists/tests_test_views.py (ch11l050)
[source,python]
----
from lists.forms import (
    DUPLICATE_ITEM_ERROR, EMPTY_LIST_ERROR,
...
And while we're at it, let's fix up our duplicate item test, so that it
becomes several different tests, including one for the form class:

[role="sourcecode"]
.lists/tests_test_views.py
[source,python]
----
    def post_invalid_duplicate_item_input(self):
        list_ = List.objects.create()
    ...
    def test_invalid_duplicate_item_error_means_nothing_saved_to_db(self):
        self.post_invalid_duplicate_item_input()
        self.assertEqual(Item.objects.all().count(), 1)

    def test_invalid_duplicate_item_input_renders_list_template(self):
        response = self.post_invalid_duplicate_item_input()
        self.assertTemplateUsed(response, 'list.html')

    def test_invalid_duplicate_item_input_renders_form_with_errors(self):
        response = self.post_invalid_duplicate_item_input()
        self.assertIsInstance(response.context['form'], ExistingListItemForm)
...
And we rename the old ones, for clarity:

----
def post_invalid_empty_item_input(self):
    list_ = List.objects.create()
    return self.client.post(
        '/lists/%d/' % (list_.id,),
        data={'text': ''}
...
def test_invalid_input_means_nothing_saved_to_db(self):
    self.post_invalid_empty_item_input()

def test_invalid_input_renders_list_template(self):
    response = self.post_invalid_empty_item_input()
