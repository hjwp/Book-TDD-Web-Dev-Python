...
Helper Functions in Functional Tests, Generic Waits, and Test File Organisation
-------------------------------------------------------------------------------

The next thing I'd like to add is a little validation of user inputs.  First, 
though, I'd like to do a little more housekeeping.  We'll split our functional
and unit tests out into several files to make them easier to keep track of.
And we'll also build a very useful helper which will allow us to apply an
explicit wait to any assertion in our functional tests.


Speccing out an FT for validation
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
NOTE: Keeping helper methods in a base `FunctionalTest` class is one useful way
    ...
    but prefers composition over inheritance.
...
NOTE: If you've never seen lambda functions in Python before, see the sidebar below.

So how would this magical `wait_for` method work?  Let's head over to 'base.py',
and make a copy of our existing `wait_for_row_in_list_table` method, and we'll
adapt it slight.  The 
...
`lambda` in Python is the syntax for making a short, throwaway function -- it
...
In our case, we're using it to transform a method call that would be called immediately
into a function that we can pass as an argument, and that can be called later,
and multiple times:
...
One FT file per user story or feature::
    Try to keep your FTs organised in a way that matches the workflow of your
    business, whether it's matching up with tickets in your issue tracker, or
    with user stories, or with whatever way you discuss features with your
    team.
...
Use a tests folder::
    ...
    +
    * Use a folder (usually called 'tests'), with a +++<i>__init__.py</i>+++.
...
A base class is a good place to keep helper methods::
    bla.
