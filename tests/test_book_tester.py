...
class CheckFinalDiffTest(ChapterTest):
    ...
    def test_ignore_secret_key(self):
        ...
        with self.assertRaises(AssertionError):
            self.check_final_diff()
        self.check_final_diff(ignore="SECRET_KEY") # should pass
        with self.assertRaises(AssertionError):
            diff += '\n+a genuinely different line'
            self.check_final_diff(ignore="SECRET_KEY")


    def test_ignore_migrations_comment_times(self):
        diff = dedent(
            """
            index 8249a47..699d098 100644
            --- a/lists/migrations/0003_list.py
            +++ b/lists/migrations/0003_list.py
            @@ -1,5 +1,5 @@
             # -*- coding: utf-8 -*-
        ...
        with self.assertRaises(AssertionError):
            self.check_final_diff()
        self.check_final_diff(ignore="migration comments") # should pass
        with self.assertRaises(AssertionError):
            diff += '\n+a genuinely different line'
            self.check_final_diff(ignore="migration comments")
