...
Let's crack on with the server side of our new auth system.  In this chapter we'll
do some more mocking, this time with Python.  We'll also find out about how to
customise Django's authentication system.
...
[role="sourcecode"]
.accounts/tests/test_views.py (ch16l012-1)
[source,python]
----
...
That test covers the desired response.  Now test that the user
actually gets logged in correctly.  We can do that by inspecting the
Django test client, to see if the session cookie has been set correctly.
...
[role="sourcecode"]
.accounts/tests/test_views.py (ch16l012-2)
[source,python]
----
...
Because HTTP is stateless, servers need a way of identifying clients with
'every single request' -- IP addresses can be shared, so the usual solution is
to give the client a unique session ID, which the client will store in a
...
the session ID by hand if you like.  It's stored under the key `sessionid` by
...
These session cookies are set for all visitors to a Django site, whether they're
logged in or not.

When we want to recognise a client as being a logged-in and authenticated,
...
being an authenticated session, and associated it with a user ID in its
database.
...
[role="sourcecode"]
.accounts/tests/test_authentication.py (ch16l022)
[source,python]
----
...
[role="sourcecode"]
.accounts/authentication.py (ch16l024)
[source,python]
----
def authenticate(self, assertion):
...
[role="sourcecode"]
.accounts/tests/test_authentication.py (ch16l045)
[source,python]
----
def test_finds_existing_user_with_email(self, mock_post):
    mock_post.return_value.json.return_value = {'status': 'okay', 'email': 'a@b.com'}
...
We create the user with the username `someone@else.com`, to make sure our
implementation doesn't just grab any old user. Now the test fails:
...
//TODO: consider implementing a repr for the user?
...
[role="sourcecode"]
.accounts/authentication.py (ch16l046)
[source,python]
----
def authenticate(self, assertion):
    [...]
