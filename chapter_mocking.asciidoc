...
((("Django framework", "sending emails")))((("emails, sending from Django")))((("mail.out box attribute")))In
...
sends by using the `mail.outbox` attribute.  ((("mocks", "benefits and drawbacks of")))But
...
NOTE: Am I telling you not to use Django's `mail.outbox`?  No; use it, it's a 
    neat shortcut.  ((("external dependencies")))But
...
'...python-tdd-book/accounts/views.py'> does not have the attribute 'auth'
----
...
*******************************************************************************
If you've used `unnittest.mock` before, you may have come across its special
`assert_called...`
https://docs.python.org/3/library/unittest.mock.html#unittest.mock.Mock.assert_called[methods], and you may be wondering why I didn't use them.
...
And the mock library will raise an `AssertionError` for you if there is a
mismatch.

Why not use that?  For me, the problem with these magic methods is it's too
...
Now is a good time to save it somewhere so we don't forget it.
...
From now on, you'll have to remember to run that `source .env` command
each time you start working on the project, a bit like activating your
virtualenv.footnote:[If you search or ask around, you'll find there are some
tools and shell plugins that load virtualenvs and _.env_ files automatically,
but maybe doing it manually is good practice, for now.]
