...
(have another look at <<spike-reminder,the spiked code above>> if you need a
reminder).
...
TIP: This is a good time to check out the 
    ...
    docs on authentication] for a little more context.
...
config in 'settings.py', you should see the explanatory traceback, as
above. It's saying something about a `last_login` field.
...
look though the django source lines listed in the traceback, and have a read up
