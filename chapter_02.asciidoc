...
<3> `setUp` and `tearDown` are special methods which get
    ...
    run even if there's an error during the test itself.
    footnote:[The only exception is if you have an exception inside `setUp`, then
    `tearDown` doesn't run.]
    No more Firefox windows left lying around!

<4> We use `self.assertIn` instead of just `assert` to make our test
    assertions. pass:[<phrase role='keep-together'><literal>unittest</literal></phrase>] provides lots of helper functions like this to make
...
It will do its job while our app is still simple, but as we'll see in Part III of
the book (e.g., in <<Persona-clientside-chapter>> and <<CI-chapter>>), you'll
want to build more sophisticated wait algorithms into your tests once your app
