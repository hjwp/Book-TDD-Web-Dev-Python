...
((("Representational State Transfer (REST)", id="rest32")))((("Representational State Transfer (REST)", "defined")))Representational State Transfer (REST) is an approach to designing a web
...
https://en.wikipedia.org/wiki/Representational_state_transfer#Relationship_between_URL_and_HTTP_methods[The
...
If we ((("Representational State Transfer (REST)", "building a REST API", id="RESTbuild32")))were building an API that was entirely agnostic about its clients, we might
...
<3> Here's the magic.  'Then' we can call `server.respond()`, whenever we like, and that
    will kick off all the async part of the Ajax loop—i.e., any callback we've assigned
...
degradation], i.e., keeping a version of the site that will still work without
JavaScript.
...
33, in test_can_start_a_list_for_one_user
...
<2> Similarly, we only start mocking our helper function 'after' we know the
    ...
    the given object with a mock version.
...
[role="sourcecode dofirst-ch36l028"]
...
Read on, Macduff.((("", startref="RESTbuild32")))


.REST API Tips
*******************************************************************************

((("Representational State Transfer (REST)", "tips for REST APIs")))Dedupe URLs::
...
Bear graceful degradation and progressive enhancement in mind::
    ...
    working without JavaScript.((("", startref="rest32")))
