...
so let's change that first by adding a new block called `extra_content`:
...
so let's pull that out into an https://docs.djangoproject.com/en/5.1/ref/templates/builtins/#include[include]
