...
if os.getenv("DJANGO_DEBUG_FALSE") == "1":  #<1>
    DEBUG = False
...
//
// Also, `DJANGO_DEBUG_FALSE` would be found in os.environ even if its value was
// `0` resulting in never reaching the `else` path.
...
// CSANAD: but now that I fixed the reachability of the development mode, this
//         following statement is no longer true:
...
[source,python]
----
if os.getenv("DJANGO_DEBUG_FALSE") == "1":
    DEBUG = False
    SECRET_KEY = os.environ["DJANGO_SECRET_KEY"]
