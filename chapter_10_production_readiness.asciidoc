...
2. To be in a directory that's writable by the nonroot user

Let's say that our nonroot user is going to be called `nonroot`,

Since they will have a writable home directory at `/home/nonroot`,
we'll put the database file in there
(which they'll have write access to of course),

Because we're mounting the file from outside though,
that's not quite enough to make it writable.
We'll need to set the owner of the file to be `nonroot` as well.
Because of the way Linux permissions work,footnote:[
...
Because /etc/passwd is not the same inside an outside the container,
...
we'll have to use integer user ids (uids).
This might seem a bit magical if you're not used to Linux permissions,
so you'll have to trust me I'm afraid.


Anyways, first let's make the path to the database file
...
RUN adduser --uid 1234 nonroot  # <2>
USER nonroot  # <3>
...
<1> We use the `adduser` command to create our user,
    explicitly setting the home directory to _/home/nonroot_, and, more importantly,
    the numerical uid to 1234.footnote:[
...
<3> The `USER` directive in the Dockerfile tells Docker to run
    everything as that user by default.

Let's test this out locally:
...
And re-run the FTs again to be sure that worked:
