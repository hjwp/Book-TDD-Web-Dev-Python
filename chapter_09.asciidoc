...
Traceback (most recent call last):
  File
"/home/harry/Dropbox/book/source/chapter_9/superlists/lists/tests/test_models.py",
line 31, in test_saving_and_retrieving_items
...
Time for a new unit test for our view:
...
That works.  Time for a commit

[subs="specialcharacters,quotes"]
----
$ *git diff*
$ *git commit*  # eg "validation implemented at views level"
----
...
TODO: add link to Django modelforms docs
...
widgets for ModelForm fields, similarly to the way we did it with the normal form:
...
`assertMultiLineEqual` is useful for comparing long strings, it gives you a
diff-style output, but it truncates long diffs by default; that's why we also
need to set `maxDiff = None` 
...
And that gets us back to passing.  At this point it's OK to delete
the old test, because it really does just duplicate the new one.  The
`assertTemplateUsed` and `response.context` checks are sufficent for
testing a basic view with a GET request.
...
TODO: tidy up URLs
TODO: switch to class-based views (or put in appendix?)
