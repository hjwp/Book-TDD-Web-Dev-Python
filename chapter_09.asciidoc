...
((("automation, in deployment", see_also="deployment")))((("automation, in deployment", id="ix_autodeploy", range="startofrange")))((("deployment", "automating", id="ix_deploymentauto", range="startofrange")))Automating deployment is critical for our staging tests to mean anything.
...
((("automation, in deployment", "installing Fabric")))((("Fabric", "installing")))Fabric is a tool which lets you automate commands that you want to run on
...
((("deployment", "sample script", id="ix_deploymentsample", range="startofrange")))((("Fabric", "sample deployment script", id="ix_Fabricsample", range="startofrange")))The best way to see how it works is with an example.
...
<3> ((("secret key")))Django uses `SECRET_KEY` for some of its crypto--cookies and CSRF
...
<5> I'm using a 'relative import' (`from .secret key` instead of `from
    ...
    more about relative imports in the next chapter.((("relative import")))
...
(((range="endofrange", startref="ix_deploymentsample")))(((range="endofrange", startref="ix_Fabricsample")))Finally, we update the database with `manage.py migrate`:
...
finally the `migrate` completes without a hitch.((("fabric", "configuration")))
...
((("deployment", "deploying to live")))So, let's try using it for our live site!
...
((("sed (stream editor)")))((("Nginx")))((("Gunicorn")))What else do we need to do to get our live site into production? We refer to
...
((("Git tags", "historical marking with")))One final bit of admin.  In order to preserve a historical marker,
...
((("deployment", "further reading on")))There's no such thing as the One True Way in deployment, and I'm no grizzled
...
(((range="endofrange", startref="ix_autodeploy")))(((range="endofrange", startref="ix_deploymentauto")))((("provisioning", "automation in")))((("Ansible")))((("automation, in provisioning")))For some ideas on how you might go about automating the provisioning step,
...
Fabric::
    ...
    is a great tool for automating server admin tasks.((("Fabric")))
...
Idempotency::
    ...
    a server that's already configured.((("Idempotency")))
...
Configuration management tools::
    ((("configuration management tools", seealso="Fabric")))((("configuration management tools")))Fabric is very flexible, but its logic is still based on scripting. More
