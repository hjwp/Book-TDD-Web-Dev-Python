...
WARNING: this section requires an upgrade to  the development branch of Django.
...
http://south.readthedocs.org/en/latest/index.html[South].  At the time of
writing however, Andrew Godwin, South's talented creator, had just finished a 
...
https://docs.djangoproject.com/en/dev/topics/migrations/[Migrations]

Accordingly, in an attempt to make this book future-proof, I'm now going to
upgrade to the development version of Django.  My expectation is that by the
time you read this, it will simply be part of the current production release
of Django.
//TODO: check this

Here's how I did it -- be warned that it takes a few minutes to pull Django
down:

[subs="specialcharacters,quotes"]
----
$ *../virtualenv/bin/pip install --upgrade https://github.com/django/django/archive/master.zip*
[...]
...
We can add it to 'requirements.txt' like this, replacing the line that used to
say `Django==1.5.1`:
...
[subs="specialcharacters,quotes"]
----
$ *git commit -am "Add Django dev version to requirements.txt"*
----
...
Do you want to enable migrations for app 'lists'? [y/N] y
...
Next we create the "real" migration that we want to apply:

[subs="specialcharacters,quotes"]
----
$ *../virtualenv/bin/python manage.py makemigrations lists*
Migrations for 'lists':
  0002_auto.py:
    - Alter unique_together for item (2 constraints)
----

Sure enough, it spots the new constraint.  Let's add that 
to VCS too:

[subs="specialcharacters,quotes"]
----
$ *git add lists/migrations/0002_auto.py*
...
TODO: check out apparent bug that occurs if you use django 1.7 to do this
migration.


Now, if you try doing a migration or a syncdb, you'll see that Django is
confused:
...
Operations to perform:
  Synchronize unmigrated apps: messages, auth, sessions, staticfiles, functional_tests, sites, contenttypes
...
Running migrations:
  Applying lists.0001_initial...Traceback (most recent call last):
  File "/home/harry/Dropbox/book/source/chapter_09/virtualenv/lib/python3.3/site-packages/django/db/backends/utils.py", line 61, in execute
    return self.cursor.execute(sql, params)
  File "/home/harry/Dropbox/book/source/chapter_09/virtualenv/lib/python3.3/site-packages/django/db/backends/sqlite3/base.py", line 477, in execute
    return Database.Cursor.execute(self, query, params)
sqlite3.OperationalError: table lists_list already exists
----
...
Operations to perform:
  Target specific migration: 0001_initial, from lists
Running migrations:
...
Eh? this is bugging out with a:

----
  File "/home/harry/Dropbox/book/source/chapter_09/virtualenv/lib/python3.3/site-packages/django/db/models/options.py", line 381, in get_field_by_name
...
Traceback (most recent call last):
  File "/home/harry/Dropbox/book/source/chapter_09/virtualenv/lib/python3.3/site-packages/django/db/models/options.py", line 384, in get_field_by_name
    return cache[name]
KeyError: 'l'
...
[...]
  ...
  File "/home/harry/Dropbox/book/source/chapter_09/virtualenv/lib/python3.3/site-packages/django/db/models/options.py", line 387, in get_field_by_name
...
https://code.djangoproject.com/ticket/21236
