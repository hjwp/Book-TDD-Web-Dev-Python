...
// JAN: For longer tests, things can become unreadable. The approach that I'm using is to have function/method for each step of the tests and then deal with assertions and internals there. Makes things much more readable. I assume it would be too much of an effort to update the code to this style but I think it would be worth at least mentioning it. I adopted the idea from Dave Farley. See this video for example: https://youtu.be/JDD5EEJgpHU?t=272
// SEBASTIAN: The pattern JAN mentions here is called DSL and that's how it's called in
//      the video linked. I see it's mentioned later in the chapter, which is great.
...
// SEBASTIAN: +1, extra word of explanation might come in handy here.
//    How about 'DSL stands for domain-specific language'. It comes down
//    to writing a class(es) that are using language of problem domain as method names.
//    That way we can easily build more test cases.
