...
We've now seen in practice all the main aspects of the TDD process:
...
where "writing the code" to get the functional tests to pass involves using
another, smaller TDD cycle which uses unit tests:
...
our application, but we can change or add and remove the unit tests, and use
a unit test cycle to actually change the implementation.

Over the coming chapters, I'm going to say things like "now run the tests", and
expect you to know which tests I mean. How do I know if I'm in a unit test /
code cycle?  Well, if I try and run the unit tests, do they all pass?  If so,
it must be time to run the functional tests again.  If any unit tests fail,
then I must be in the unit test / code cycle...
