...
((("functional tests (FTs)", "ensuring isolation", id="FTisolat23")))In the last chapter, we made the decision to leave a unit test failing in
...
((("isolation, ensuring", "failed test example")))Let's revisit the point we were at half-way through the last chapter, when we
...
<3> Finally, we make sure that the function with the `side_effect` was
    actually triggered, i.e., we did `.save()`.  Otherwise our assertion
...
Decoupling our application from ORM code::
    ((("Object-Relational Mapper (ORM)")))When striving to write isolated tests, one of the consequences is that we
