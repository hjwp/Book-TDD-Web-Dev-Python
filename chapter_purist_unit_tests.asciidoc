...
To follow along with this chapter, it’s probably easiest to reset your code to match my example code as it was in the 2e, by resetting to: https://github.com/hjwp/book-example/tree/chapter_server_side_debugging
...
In the preceding chapter, we made the decision to leave a unit test failing in the views layer
...
Bernhardt, who took a look at an early draft of the previous chapter, and
...
NOTE: I revisited some of the tradeoffs outlined in this chapter in my
...
We'll actually go back in time and check out the old codebase
using the tag we saved earlier, so that we can see
how things would have worked if we'd used more isolated tests:

// TODO: change tag name to be less leading
...
NOTE: You won't see this error unless you actually check out the old code
    and revert _lists/models.py_.
    You should definitely do this;
    part of the objective of this chapter is to see
    whether we really can write tests for a models layer
...
((("isolation, ensuring", "using mocks for", secondary-sortas="mocks for", id="IEmock23")))
((("mocks", "isolating tests using", id="Misolate23")))
Lists don't have owners yet,
but we can let the views layer tests pretend they do
by using a bit of mocking:
...
[...]
    ...
    @mock.patch("lists.views.List")  # <1>
    @mock.patch("lists.views.ItemForm")  # <2>
    def test_list_owner_is_saved_if_user_is_authenticated(
        self, mockItemFormClass, mockListClass  # <3>
    ):
        mock_list = mockListClass.objects.create.return_value
        mock_list.get_absolute_url.return_value = "/fake-url"  # <3>
...
<4> The list instance that the view will have access to
    will be the return value of the mocked `List` class's
    `objects.create` method.

<5> We also need to provide a url-like value
    for our mock list's `get_absolute_url()` method,
    otherwise the call to `redirect()` later in the view will crash.
    Did I mention that once you start mocking, it can be hard to stop?


<5> Finally, we can make assertions about whether the `.owner` attribute
    is set on it on our mock list, after the view has been called.
...
exactly as I've shown it, using `List.objects.create()`, not `List()`.
This is an illustration of the way that
mocks/monkeypatching leaves you tightly coupled to your implementations.


NOTE: Using mocks does tie you to specific ways of using an API.
      This is one of the many trade-offs involved in the use of mock objects.
