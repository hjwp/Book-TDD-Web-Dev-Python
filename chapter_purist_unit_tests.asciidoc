...
|=======
2+|Chapter info
|shortname:|chapter_purist_unit_tests
|=======
...
We'll actually go back in time and check out the old codebase using the tag
we saved earlier, so that we can see how things would have worked if we'd used
more isolated tests.
...
<2> We also mock out the `ItemForm`, because otherwise our form will
...
So strictly speaking we need to check, not just that the owner is assigned, but
that it's assigned 'before' we call save on our list object.
...
TIP: Two common mistakes when using mock side-effects are:  assigning the
    ...
    by common, I mean, ``I made both these mistakes several times 'while writing
    this chapter'''.
...
(3) is tested explicitly.  On items (1) and (2) we were lucky--they're default
