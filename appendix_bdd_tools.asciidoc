...
NOTE: since writing this chapter, one of my readers (thanks Jared!) pointed me
    towards an alternative to django-behave called "behave-django" (naturally).
    From what I can tell, it removes some of the shortcomings of django-behave.
    I'm sold, you should check it out: https://pythonhosted.org/behave-django/
...
from functional_tests.management.commands.create_session import create_pre_[...]
...
----
$ pass:quotes[*./manage.py test bdd_tests --testrunner=django_behave.runner.DjangoBehaveTestSuiteRunner*]
...
Feature: My Lists # bdd_tests/features/my_lists.feature:1
  ...
  Scenario:                                                   # my_lists.feature:2
    ...
    Given I am a logged-in user                               # steps/my_lists.py:7
    When I create a list with first item "Reticulate Splines" # steps/my_lists.py:20
...
Feature: My Lists # bdd_tests/features/my_lists.feature:1
  ...
  Scenario: Create two lists and see them on the My Lists page  # my_lists.feature:6
    ...
    Given I am a logged-in user                                 # steps/my_lists.py:7
    When I create a list with first item "Reticulate Splines"   # steps/my_lists.py:20
    And I add an item "Immanentize Eschaton"                    # steps/my_lists.py:27
    And I create a list with first item "Buy milk"              # steps/my_lists.py:20
    Then I will see a link to "My lists"                        # steps/my_lists.py:33
      Traceback (most recent call last):
        [...]
...
user-centric thinking. (There is a syntax in Gherkin for including "comments" in a feature
...
any firm conclusions. I find the "forced" structuring of FTs into steps very appealing though--it looks like it has the potential to encourage a lot of reuse in your
...
lines of steps code. Overall, I would say it's definitely worth investigating, and I will 
