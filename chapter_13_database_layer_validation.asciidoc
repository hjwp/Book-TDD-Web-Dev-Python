...
// DAVID: Surely there's a better link than something from Python 2.5,
// which contains Python examples that aren't compatible with Python 3?
// Some readers may not be aware of the differences and could trip them up.
...
// DAVID: This wrong-footed me a bit -  I ran the test and found it failed, and was confused.
// Why not instead get them to run the test, watch it fail, and then go into an aside about this?
// But in general this whole bit feels like it's turning red-green-refactor on its head, it's a slightly confusing message.
// We're not trying to write a test that passes, we're trying to write one that fails, surely?
...
// DAVID: Would be good to provide the command to run the unit test.
...
requests.
// DAVID did you? Looks like the same name to me?


// DAVID: we switched to running the unit test, would be good to provide the command.
That gives:
...
// DAVID: For some reason they failed when I ran it because of a time.sleep() in test_simple_list_creation.py
// that I needed to delete. Probably just a mistake I made earlier but flagging just in case.
...
// DAVID: Again, would be nice to show the command.
...
// Might be better to check it against reverse("view_list", args=[mylist.id]) - or just assert it's url that
/ can be passed to `resolve`, or even just a string.
...
// DAVID: Are you, though? This is only validation at the Python layer, not the database layer.
// Better to test against some SQL that tries to insert it?
