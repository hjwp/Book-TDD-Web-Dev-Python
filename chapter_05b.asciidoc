...
Avoiding race conditions by making selenium calls atomic
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^


We start by rewriting our `check_for_row_in_list_table` function to only
call selenium once. The reason is that, if we first retrieve the table
and then retrieve its rows, we set up a "race condition" where Selenium could
conceivably fetch the table object from the page before the load, and then
try and fetch the rows as the new page loads, and then we'll get errors
(`StaleElementReferenceException` usually).  By making just one "atomic"
call to Selenium, we avoid that race condition:
