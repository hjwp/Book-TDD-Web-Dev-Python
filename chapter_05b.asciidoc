...
A `wait_for` decorator
^^^^^^^^^^^^^^^^^^^^^^

We'll implement our waits using a very Pythonic tool: a decorator.  Now, if
you're new to Python, this is definitely an intermediate-level technique, so
you have two choices.  Either spend a good bit of time wrapping your head
around the functional programming techniques involved, until you're sure you
understand it.  Or, put that off to another day, and trust that it "just works",
because once we've written our decorator, as you'll see, using it will be
very easy.  That's the Pythonic part!

OK hold on to your functional hat, here we go!
...
def wait_for(fn):  #<1>
    def modified_fn(*args, **kwargs):  #<2>
        start_time = time.time()
        while time.time() - start_time < 5:  #<3>
            try:
                return fn(*args, **kwargs)  #<3>
            except (AssertionError, WebDriverException):  #<4>
                time.sleep(0.1)
        # one more try, which will raise any errors if they are outstanding
        return fn(*args)  #<5>
    return modified_fn  #<6>
----

<1> A decorator is a way of modifying a function, and itself is a function
    that takes a function as an argument, and returns another function as
    the modified version.

<2> Here's where we create our modified version of the function. It
    can take arbitrary arguments (`args` and `kwargs`).

<3> Here's the loop, which will keep going until our timeout expires

<4> We try and call our function, and as long as it doesn't raise any
