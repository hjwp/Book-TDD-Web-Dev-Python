...
((("unit tests", "refactoring")))((("refactoring", "unit tests")))We're going to want to add another test for our model, but before we
...
((("context managers")))((("unit tests", "context manager")))Let's add a new test method to `ListAndItemModelsTest`, which tries to create
a blank list item:
...
((("errors", "in model validation")))Let's try and enforce our model validation in the views layer and bring it up
...
----
(((range="endofrange", startref="ix_validationml")))(((range="endofrange", startref="ix_Djangomlval")))(((range="endofrange", startref="ix_mlvalidation")))

Django Pattern: Processing POST Requests in the Same View as Renders the Form
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

((("Django", "POST requests", see="POST requests")))((("POST requests", "processing", id="ix_postreqproc", range="startofrange")))This time we'll use a slightly different approach, one that's actually a very
...
skip to the current FT.(((range="endofrange", startref="ix_refactorapp")))
...
tea break.(((range="endofrange", startref="ix_postreqproc")))
