...
[subs="specialcharacters,quotes"]
----
elspeth@server:$ *echo EMAIL_PASSWORD=sekrit >> ~/sites/superlists-staging.ottg.eu*
elspeth@server:$ *sudo systemctl daemon-reload*
elspeth@server:$ *sudo systemctl restart gunicorn-superlists-staging.ottg.eu*
elspeth@server:$ *sudo journalctl -f -u gunicorn-superlists-staging.ottg.eu*
----
...
def _get_server_env_vars(host):
    env_lines = run(f'cat ~/sites/{host}/.env').split('\n')
...
Upating our Provisioning Notes
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


((("debugging", "server-side", "baking in logging code")))Before
we finish, let's make a note about the update to our provisioning
steps:
