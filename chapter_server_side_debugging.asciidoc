...
((("debugging", "server-side", "using staging sites", tertiary-sortas="staging sites", id="DBserstag21")))((("staging sites", "catching final bugs with", id="SScatch21")))They're all very well for running the FTs locally, but how would they work
against the staging server?  Let's try and deploy our site.  Along the way
...
((("logging")))((("Gunicorn", "logging setup")))In order to track this problem down, we have to set up Gunicorn to do some
...
((("debugging", "server-side", "setting secret environment variables")))((("environment variables")))((("secret values")))In <<chapter_automate_deployment_with_fabric>>, we've seen one way of setting secret
...
((("debugging", "server-side", "managing test databases", id="DBservdatabase21")))((("staging sites", "managing test databases", id="SSmanag21")))((("database testing", "managing test databases", id="DTmanag21")))((("sessions, pre-creating")))Now we can rerun our FTs, and get to the next failure: our attempt to create
...
((("scripts, building standalone")))To do things on the server, we'll need to build a self-contained script that
...
((("Fabric", "using directly from Python")))Rather than using the `fab` command, Fabric provides an API that lets
...
((("staging sites", "local vs. staged sessions")))Does that all make sense?  Perhaps a little ascii-art diagram will help:
...
*******************************************************************************
((("database testing", "safeguarding production databases")))((("production databases")))We're into dangerous territory, now that we have code that can directly
...
((("debugging", "server-side", "baking in logging code")))Before we finish, let's "bake in" our logging setup. It would be useful to 
...
of working to get through them, but we've ended up with several useful things
as a result.
...
Fixtures also have to work remotely::
    `LiveServerTestCase` ((("fixtures", "staging and")))((("staging sites", "fixtures and")))makes it easy to interact with the test database 
...
Be very careful when resetting data on your servers::
    ((("database testing", "safeguarding production databases")))((("production databases")))A command that can remotely wipe the entire database on one of your
...
Logging is critical to debugging issues on the server::
    ((("logging")))((("debugging", "server-side", "baking in logging code")))At the very least, you'll want to be able to see any error messages
