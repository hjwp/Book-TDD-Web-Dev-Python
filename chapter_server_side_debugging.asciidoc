...
@@ -60,13 +64,11 @@ class LoginTest(FunctionalTest):
         ))
...
@@ -76,11 +78,11 @@ class LoginTest(FunctionalTest):
...
//TODO: fix this listing, no setupclass
    ...
    def setUpClass(cls):
        for arg in sys.argv:
            if 'liveserver' in arg:
                ...
                cls.live_server_url = 'http://' + cls.server_host
...
<1> Instead of just storing `cls.live_server_url`, we also store the `server_host` 
    attributes if we detect the `liveserver` command-line argument. //TODO: fixme
...
specification of the `STAGING_SERVER` variable now:

[role="skipme"]
[subs="specialcharacters,macros"]
----
$ pass:quotes[*STAGING_SERVER=elspeth@superlists-staging.ottg.eu python manage.py test \
