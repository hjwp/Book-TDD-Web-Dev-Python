...
something wrongfootnote:[Although you can come pretty close. We are covering
the essentials here, which alone is quite sufficient for writing nice,
production-ready code. Once you're not too overwhelmed by writing tests
manually, take a look at the
https://hypothesis.readthedocs.io/en/latest/[Hypothesis] library! It helps you
set up rules for generating input for your tests, essentially covering many
more test scenarios than would make sense typing manually.].  If you have a
function that adds two numbers, you can write a couple of tests:
...
// https://github.com/hjwp/book-example/tree/chapter_15_advanced_forms if that
// helps?
