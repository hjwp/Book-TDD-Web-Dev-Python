...
oh gosh, I think that bit over there inherits from it--no, it doesn't, it's overridden.
...
with a company called Resolver Systems (now https://www.pythonanywhere.com[PythonAnywhere]), where
...
=== Outline
// RITA: Remember to update this to refer to the four parts.
