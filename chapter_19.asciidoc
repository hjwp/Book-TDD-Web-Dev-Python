...
TIP: Have you heard the term "integration test" and are wondering what the
    ...
    definitions box in <<hot-lava-chapter>>.((("integrated tests", "vs. integration test", sortas="integration")))
((("integration tests")))
...
<1> The mocked `form.save` function is returning an object, which we expect
    our view to be able to use.((("contracts, implicit")))
...
your code lie.((("refactoring", "and test isolation", sortas="testisolation")))
((("test isolation", "refactoring in")))
...
wonder if there's something we can do about that?(((range="endofrange", startref="ix_integratedtests")))
