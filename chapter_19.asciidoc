...
NOTE: I would appreciate feedback about this chapter.  Did you find it too hard?
...
An exercise for the reader: Overview
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

To implement this, we'll need:

* A new section in 'list.html', with, to start with, an email input box
  and form.

* Next, we'll need a view for the form to submit to. Start by defining the
  URL in the template, maybe something like 'lists/<list_id>/share', with
  just a placeholder view function

* we can then write a unit test for the view which creates a user, 
  does a POST with their email address, and checks the user is added to
  a `list_.shared_with.all()` (a similar ORM usage to "my lists")


* a check that the view redirects after the POST is also a good idea.

* Before we can get this test to pass, we have to move down one layer,
  to the models layer.  Something like this:
...
* You'll then need a ManyToManyField.  You'll see an error message about
clashing related_names, which you'll find a solution to if you look around
the Django docs.
...
* Add a csrf token to the form if you haven't already

* then back up to the template level, put a for loop of the lists shared with, 
  and mention the list owner on a lists page
