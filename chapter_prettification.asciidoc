...
((("Bootstrap", "documentation")))If
we have a look at the "Getting Started" section of the 
...
item 1, Django lets us define a URL "prefix" to say that any URLs which
start with that prefix should be treated as requests for static files.  By
default, the prefix is pass:[<span class="keep-together"><code>/static/</code></span>]. It's defined in 'settings.py':
...
you deploy your code to pass:[<span class="keep-together">production</span>].
