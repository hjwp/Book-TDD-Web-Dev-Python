...
`LiveServerTestCase` doesn't. Never fear, though:
the Django developers have made a more magical test class
...
image::images/prettified-dark.png["Screenshot of lists page in dark mode."]

But it's very much a matter of personal preference,
and my editor will kill me if I make all the rest of my screenshots use
so much ink, so I'm going to revert it for now.
...
I _definitely_ recommend taking the time to do that some day.
Sass/SCSS is a great improvement on plain old CSS,
and a useful tool even if you don't use Bootstrap.
I won't cover it in this book, but you can find resources on the internets.
https://www.freecodecamp.org/news/how-to-customize-bootstrap-with-sass/[Here's one],
for example.


A last run of the functional tests, to see if everything still works OK:

[role="dofirst-ch08l019"]
[subs="specialcharacters,macros"]
----
$ pass:quotes[*python manage.py test functional_tests*]
[...]
...
using `pathlib.Path` and `__file__` (both really nice Python builtins)footnote:[
Notice in the `Pathlib` wrangling of `__file__`
that we do a `.resolve()` before anything else.
...
It's one of Django's powerful features, and we'll find out all about it one day,
but we're not ready to use that yet, so let's disable it for now:
