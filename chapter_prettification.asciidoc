...
((("static files", "URL requests for")))For item 1, Django lets us define a URL "prefix" to say that any URLs which
...
((("", startref="DJFstatic08")))And now it will now find the new CSS, which will get our test to pass:
...
((("design and layout testing", "collecting static files for deployemnt", id="DLTcollect08")))((("static files", "collecting for deployment", id="SFcollect08")))((("collectstatic command", id="collect08")))We saw earlier that the Django dev server will magically find all your static
...
* The `{% static %}` template tag, for more DRY and less hardcoded URLs
