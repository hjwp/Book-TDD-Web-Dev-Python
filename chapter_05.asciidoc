...
((("user input, saving", id="ix_userinput", range="startofrange")))We want to take the to-do item input from the user and send it to the server,
...
((("POST requests", "sending", id="ix_POSTrequestsending", range="startofrange")))At the end of the last chapter, the tests were telling us we weren't able to
...
((("debugging")))When a functional test fails with an unexpected failure, there are several
...
((("'time.sleep'")))We'll look at all of these over the course of this book, but the `time.sleep` 
...
((("CSRF error")))((("Cross-Site Request Forgery (CSRF)")))((("Anderson, Ross")))((("'Security Engineering' (Anderson)")))((("template tag")))Django's CSRF protection involves placing a little auto-generated token into
...
ignores it and displays the normal home page.(((range="endofrange", startref="ix_POSTrequestsending")))
...
((("POST requests", "processing")))Because we haven't specified an `action=` attribute in the form, it is
...
((("templates", "Python variables in", id="templatespythonvar", range="startofrange")))We've already had a hint of it, and now it's time to start to get to know the real
...
((("render to string")))As you can see, the `render_to_string` function takes, as its second parameter,
...
((("tracebacks")))If you remember the rules for reading tracebacks, you'll spot that it's
...
((("debugging")))Hmm, not a wonderfully helpful error.  Let's use another of our FT debugging
...
((("AssertionError")))That gives us a more helpful error message:
...
((("triangulation")))((("duplication, eliminate")))((("'eliminate duplication'")))((("refactoring")))((("Red, Green, Refactor")))(((range="endofrange", startref="ix_templatespythonvar")))Now we get to the `self.fail('Finish the test!')`.  If we extend our FT to 
...
once you get three occurrences, it's time to remove duplication.((("'don't repeat yourself' (DRY)")))
...
((("helper methods")))Back to our functional test refactor: we could use an inline function, but that
...
((("ORM (Object-Relational Mapper)", id="ix_ORM", range="startofrange")))((("Django", "Object-Relational Mapper (ORM)", id="ix_Django_ORM", range="startofrange")))((("Object-Relational Mapper (ORM)", id="ix_objectrelationalmapper", range="startofrange")))An Object-Relational Mapper (ORM) is a layer of abstraction for data stored in 
...
information was saved.((("'QuerySet'")))
...
tutorial], which has an excellent intro to them. ((("unit tests", "vs. integrated tests")))((("integrated tests", "vs. unit tests")))
...
((("database migration", id="ix_databasemigration", range="startofrange")))((("migration, database", id="ix_migrationdatabase", range="startofrange")))The next thing that happens is a database error:
...
((("Django", "field types")))Django has many other field types, like `IntegerField`, `CharField`,
...
(((range="endofrange", startref="ix_migrationdatabase")))(((range="endofrange", startref="ix_databasemigration")))(((range="endofrange", startref="ix_Django_ORM")))(((range="endofrange", startref="ix_ORM")))(((range="endofrange", startref="ix_objectrelationalmapper")))So let's do a commit for our first ever model!
...
((("POST", "saving to database", id="ix_POSTsaving", range="startofrange")))Let's adjust the test for our home page POST request, and say we want the view
...
(((range="endofrange", startref="ix_POSTsaving")))And that gets the test passing:
...
((("POST", "redirect after")))((("redirect, after a POST")))But, yuck, that whole `new_item_text = ''` dance is making me pretty unhappy.
...
((("templates", "rendering items in", id="ix_templatesrendering", range="startofrange")))((("Django", "templates", id="ix_Djangotemplates", range="startofrange")))Much better!  Back to our to-do list:
...
them in the 
https://docs.djangoproject.com/en/1.7/topics/templates/[Django docs].
...
page saying "no such table: lists_item", as in <<operationalerror>>.(((range="endofrange", startref="ix_templatesrendering")))(((range="endofrange", startref="ix_Djangotemplates")))
...
((("'migrate'", id="ix_migrate", range="startofrange")))((("Django","migrate", id="ix_Djangomigrate", range="startofrange")))Another helpful error message from Django, which is basically complaining that
...
((("superusers")))I said "no" to the question about superusers--we don't need one yet, but we
...
(((range="endofrange", startref="ix_migrate")))(((range="endofrange", startref="ix_Djangomigrate")))Grrr.  We're so close! We're going to need some kind of automated way of
...
((("commits")))Apart from that little bug in our functional testing, we've got some code
...
Ah well.(((range="endofrange", startref="ix_userinput")))
