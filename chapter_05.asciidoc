...
++{% ... %}++&8212;famous for being the world's most annoying two-key touch-typing
combination:
...
and add a new method to ++HomePageTest++&8212;I copied the previous method,
...
pass:[<?dbhtml orphans="4"?>]Let's adjust the test for our home page POST request, and say we want the view to save a new item to the database instead of just passing it through to its response. We can do that by adding three new lines to the existing test called
...
different things.  That's another 'code smell'&8212;a long unit test either needs
