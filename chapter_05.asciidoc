...
((("Cross-Site Request Forgery (CSRF) error")))((("Anderson, Ross")))((("'Security Engineering' (Anderson)")))((("template tag")))Django's CSRF protection involves placing a little auto-generated token into
...
((("templates", "Python variables in", id="ix_templatespythonvar", range="startofrange")))We've already had a hint of it, and now it's time to start to get to know the real
...
((("triangulation")))((("duplication", "eliminating")))((("'eliminate duplication'")))((("refactoring")))((("Red, Green, Refactor")))(((range="endofrange", startref="ix_templatespythonvar")))Now we get to the `self.fail('Finish the test!')`.  If we extend our FT to 
...
((("migrations", id="ix_migrations", range="startofrange")))The next thing that happens is a database error:
...
(((range="endofrange", startref="ix_migrations")))(((range="endofrange", startref="ix_Django_ORM")))(((range="endofrange", startref="ix_ORM")))(((range="endofrange", startref="ix_objectrelationalmapper")))So let's do a commit for our first ever model!
...
((("POST requests", "saving to database", id="ix_POSTsaving", range="startofrange")))Let's adjust the test for our home page POST request, and say we want the view
...
((("POST requests", "redirect after")))((("redirect, after POST request")))But, yuck, that whole `new_item_text = ''` dance is making me pretty unhappy.
...
((("migrations", id="ix_migrate", range="startofrange")))((("Django","migrations", id="ix_Djangomigrate", range="startofrange")))Another helpful error message from Django, which is basically complaining that
