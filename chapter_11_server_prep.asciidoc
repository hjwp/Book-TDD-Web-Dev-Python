...
Our objective is to use Ansible((("Ansible", "interacting with server using SSH and Ansible")))((("SSH", "using with Ansible to interact with server"))) to automate the process of deploying to our server:
...
a sort of mini-Linux that Microsoft has made to run inside Windows.((("Windows Subsystem for Linux (WSL)")))
