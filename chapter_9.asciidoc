...
That would work, but we're really doing too much work in our view. Django
...
is to iterate towards the ideal form, and hopefully introduce forms gradually
...
That gives us a fail.  How can we customise the input for a form field?  Using 
a "widget":
...
That gives us some different-looking form HTML:
...
input, and that won't work for our app. Thankfully, you can override widgets
for ModelForm fields, similarly to the way we did it with the normal form:
...
which we defined on the model.  We'll also learn how to pass data into
the form, as if it came from the user:
...
And the tests still pass. great
...
There's a sort of consort to the "deploy as early as possible" lean methodoloy,
which is "merge as early as possible".  What I mean by that is that, in building
...
Let's start in our unit tests for the home view. Let's replace the old-stype
...
`{{form.text}}` renders just the `text` field of the form.


Now our old test fails is out of date:
...
----
      <form method="POST" action="/lists/new" >
-         <input id="id_text" name="text" placeholder="Enter a to-do item" type="text" />
+         
----
...
And that gets us back to passing.
...
'test_simple_list_creation.py', 1 in 'test_layout_and_styling.py' and 6
in 'test_list_item_validation.py`:
...
of the code in line with the change.  We need to find any occurrences
of the old id (`id_new_item`) and name (`item_text`) and replace them too
...
We re-run the unit tests to check everything works
...
is absolutely critical to our app.  We've touched 7 or 8 different files, 
doing a refactor that's quite involved... This is the kind of thing that,
without tests, would seriously worry me.  In fact, I might well have decided
that it wasn't worth messing with code that works...  But, because we have
a full tests suite, we can delve around in it, tidying things up, safe in the
knowledge that the tests are there to spot any mistakes we make.  It just
makes it that much likelier that you're going to keep refactoring, keep
...
Now to use the rest of the features of our form:  the validation logic.  This
means using the form in the `new_list` view. Let's implement it straight away,
and see if the unit tests like it:
...
Did you notice that we've also fixed a small bug?  In the previous code, we
were saving a superfluous List object, even for invalid inputs.  They would
have been left lying around our database.  We should add an extra assert in 
our unit tests once this refactor is done.
...
Ah, we haven't yet put the error in our template.  Let's do that:
...
Neat!  And it gets our unit tests back to passing... And the functional tests
too! 

TODO: use errors string constants
...
We use a couple of special Django methods here, `self.non_field_errors()`, which
is just the same as +self.errors['__all__']+, but more readable, and 
...
Just to stay neat and tidy, let's say we want to remove that `__all__` non-field
error.  Adjust the unit test:
...
That gives us failures for the two validation tests.  We adjust the template:
...
Next we want to make sure that the list field is pre-populated with the 
particular list we're working on

Next we want to be able to 
The next stage is to b
