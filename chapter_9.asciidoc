...
Binding the form to a list
^^^^^^^^^^^^^^^^^^^^^^^^^^

The next error we want to catch is duplicate items, but in order to do that,
the form needs to know which list it pertains to.  Let's add 'list' as a
field on the form, but make it a hidden one, so the user can't see it.

The simplest way to do this is to reintroduce our little self.fail from 
earlier:
...
Then we add `'list'` to the fields on the form:

    class Meta:
        model = Item
        fields = ('text', 'list')

Running the tests will show us that it's using a `select` as the 
list field.  

----
AssertionError: <p><label for="id_text">Text:</label> <input id="id_text" name="text" placeholder="Enter a to-do item" type="text" /></p>
<p><label for="id_list">List:</label> <select id="id_list" name="list">
<option value="" selected="selected">---------</option>
</select></p>
----

Let's use a hidden widget instead:
...
----
AssertionError: <p><label for="id_text">Text:</label> <input id="id_text"
name="text" placeholder="Enter a to-do item" type="text" /><input id="id_list"
name="list" type="hidden" /></p>
----

And then we can pull that out into the unit test:
...
NOTE: TDD isn't for absolutely everything.  When you're learning a new API,
or just exploring possible solutions, you don't have to start with tests.
