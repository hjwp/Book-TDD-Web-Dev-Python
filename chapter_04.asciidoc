...
((("test-driven development (TDD)", "trivialities of",id="ix_TDD trivialies",range="startofrange")))I'm imagining several of you, dear readers, have been holding back
...
**********************************************************************
range="endofrange", startref="ix_TDD trivialies"
...
((("user interaction testing", id="ix_ui_testing", range="startofrange")))((("Selenium", "for user interaction testing",id="ix_Selenium_for_ui_testing",range="startofrange")))Where were we at the end of the last chapter? Let's rerun the test and find
out:
...
((("'any' function")))Also, just look at that `any` function. It's a little-known Python built-in.
I don't even need to explain it, do I? Python is such a joy.

((("'generator expression")))((("list comprehension")))Although, if you're one of my readers who doesn't know Python, what's happening
...
(((range="endofrange", startref="ix_Selenium_for_ui_testing")))(((range="endofrange", startref="ix_ui_testing"")))Big changes to a functional test are usually a good thing to commit on their
...
((("'Don't Test Constants' rule")))Let's take a look at our unit tests, 'lists/tests.py'.  Currently we're looking
...
Let's use that.(((range="endofrange", startref="ix_Selenium_for_user_interaction_testing")))


Refactoring to Use a Template
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

((("templates", "for refactoring", id="ix_templates_for_refactoring",range="startofrange")))((("refactoring", "with templates", id="refactoring_with_templates",range="startofrange")))What we want to do now is make our view function return exactly the same HTML,
...
we're comfortable with how everything works. No magic!(((range="endofrange", startref="ix_templates_for_refactoring")))(((range="endofrange", startref="ix_refactoring with templates")))

On Refactoring
~~~~~~~~~~~~~~

((("refactoring")))That was an absolutely trivial example of refactoring. But, as Kent Beck puts
it in 'Test-Driven Development: By Example', "Am I recommending that you actually work this way? No. I'm recommending that you be 'able' to work this way."
...
((("refactoring cat")))There's always a tendency to skip ahead a couple of steps, to make a couple of
...
((("'any' function")))((("'assertTrue' function")))Slightly cryptic. We can use the line number to track it down, and it turns out
...
((("AssertionError")))But now, to get this to pass, we will need to actually process the user's
...
((("Test-driven development", "process recap", id="ix_TDD recap",range="startofrange")))We've now seen all the main aspects of the TDD process, in practice:
...
((("double-loop-TDD")))But how does this apply when we have functional tests 'and' unit tests?  Well,
...
over the coming chapters.((("code", "checking")))
...
(((range="endofrange", startref="ix_TDD_recap")))
