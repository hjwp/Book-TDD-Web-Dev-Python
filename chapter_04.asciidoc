...
way you don't have to be smart 'all' the time (just remember <<figure4-1>>).
...
Create a directory called 'lists/templates' to keep templates in, and then open a file at 'lists/templates/home.html', to which we'll transfer our HTML:footnote:[Some people like to use another subfolder named after the app (i.e., 'lists/templates/lists') and then refer to the template as 'lists/home.html'.  This is called "template namespacing." I figured it was overcomplicated for this small project, but it may be worth it on larger projects.  There's more in the http://bit.ly/1iXxWZL[Django tutorial].]
...
[quote, Kent Beck, Test-Driven Development: By Example]
...
image::images/twdp_0402.png[An adventurous cat, trying to refactor its way out of a slippery bathtub, scale="40"]
