...
//TODO: investigate new Python 3 version?
...
with f-strings instead of the `os.path.join` command we saw earlier, it's because
...
<2> Many commands start with a `cd` in order to set the current working
    ...
    "cd" command, but I figured it was one thing too many to add in this chapter.]
...
def _update_settings(source_folder, site_name):
    ...
    sed(settings_path,
        'ALLOWED_HOSTS =.+$',
        f'ALLOWED_HOSTS = [{site_name!r}]'  #<2><3>
    )
    secret_key_file = source_folder + '/superlists/secret_key.py'
    if not exists(secret_key_file):  #<4>
        chars = 'abcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*(-_=+)'
        key = ''.join(random.SystemRandom().choice(chars) for _ in range(50))
        append(secret_key_file, f"SECRET_KEY = {key!r}")
    append(settings_path, '\nfrom .secret_key import SECRET_KEY')  #<5><6>
----
...
<3> The `{variable!r}` f-string syntax means insert the "repr" of the
    variable, in this case it will wrap it in the right kind of quotes.
    It's a common pattern when generating Python code as strings.

<4> Django uses `SECRET_KEY` for some of its crypto--cookies and CSRF
...
<5> `append` just adds a line to the end of a file. (It's clever enough not to
...
<6> I'm using a 'relative import' (`from .secret_key` instead of `from
...
<1> You can split long strings across multiple lines like this in Python, they
    concatenate to a single string. It's a common source of bugs when what you actually
    wanted was a list of strings, but you forgot a comma!
...
Then we write the Systemd service:
...
elspeth@server: *sed "s/SITENAME/superlists.ottg.eu/g" \
    ...
    /etc/systemd/system/gunicorn-superlists.ottg.eu.service*
----
