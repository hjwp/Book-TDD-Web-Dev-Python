...
[elspeth@superlists-staging.ottg.eu] out: Unpacking objects: 100% [...]
done.
...
Provisioning: A new .env file, plus Nginx and Gunicorn Config Using sed
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
our provisioning notes, which tell us how to generate a _.env_ file, and
gives us template files to create our Nginx virtual host and the Systemd
service.

First we create our .env file:

[role="server-commands"]
[subs=""]
----
elspeth@server:$ <strong>cd ~/sites/superlists.ottg.eu</strong>
elspeth@server:$ <strong>echo DJANGO_DEBUG_FALSE=y &gt;&gt; .env</strong>
elspeth@server:$ <strong>echo SITENAME=superlists.ottg.eu &gt;&gt; .env</strong>
...
elspeth@server:$ *cat ./deploy_tools/nginx.template.conf \
    ...
    | sed s/SEKRIT/ommffan3x1qn28xmusukyg66t0ajzm7x9ufh2teaqjhuap8vib/ \
