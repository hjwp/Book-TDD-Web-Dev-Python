...
def _create_dotenv_file_if_necessary():
    if exists('.env'):
        return  #<1>
    append('.env', 'DJANGO_DEBUG_FALSE=y')  #<2>
...
<1> We set an early return if the file already exists--this is only
    really meant to run once.


<2> The `append` command adds a line to a file.

<3> We can use our little one-liner to create the secret key in here
    too, saving us from having to copy and paste it to the command-line
...
[elspeth@superlists.ottg.eu] run: echo 'SITENAME=superlists.ottg.eu' >> "$(echo .env)"
