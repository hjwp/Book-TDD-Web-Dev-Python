...
((("Fabric", "deployment script", id="Fscript11")))The best way to see how it works is with an example.
...
((("secret values")))Next we update our settings file, to set the `ALLOWED_HOSTS` and `DEBUG`
...
((("Fabric", "running on staging site", id="Fstage11")))Let's try it out on our existing staging site, and see it working to update
...
*******************************************************************************
((("Fabric", "configuration")))If you are using an SSH key to log in, are storing it in the default location,
...
((("Fabric", "documentation")))Or see the http://docs.fabfile.org[Fabric documentation] for more info.
...
((("Nginx", "congifuring using sed")))((("Gunicorn", "congifuring using sed")))((("sed (stream editor)")))What else do we need to do to get our live site into production? We refer to
...
((("Git", "tagging releases")))One final bit of admin.  In order to preserve a historical marker,
...
((("Fabric", "additional resources")))((("automated deployment", "additional resources")))There's no such thing as the One True Way in deployment, and I'm no grizzled
...
Idempotency::
    ((("idempotency")))If your deployment script is deploying to existing servers, you need to
