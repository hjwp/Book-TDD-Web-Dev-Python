...
New columns on existing tables require default values
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

And once again we need a migration, but this time it asks us a question:
...
Select an option: pass:quotes[*1*]
...
Django is telling us that, when we add a new column to an existing table,
it needs to be given a default value to assign to any existing items in
the table.  We choose option 1 to enter a value manually, and then type
the empty string, `''`.
...
We're not quite there. Django has only saved the string representation of the
list object. To save the relationship to the object itself, we tell Django
about the relationship between the two classes using a `ForeignKey`:
...
Let's delete the last migration and try again

[subs="specialcharacters,macros"]
----
$ pass:quotes[*rm lists/migrations/0003_item_list.py*]
$ pass:quotes[*python3 manage.py makemigrations*]
You are trying to add a non-nullable field 'list' to item without a default;
we can't do that (the database needs something to populate existing rows).
Please select a fix:
...
It doesn't matter too much what we choose as the default value here, because
we haven't yet got any "real" data in a "real" database that we care about.
Once we have deployed our site to real users, we'll need to be more careful.

For now we'll make a concession towards having sane defaults by using the
integer `1` as our default, because that's at least potentially a valid 
primary key for a list object.
...
Oh dear!  There is some good news which is that, although it's hard to
see, our model tests are passing.  But three of our view tests are
failing nastily.
...
[subs="specialcharacters,quotes"]
----
$ *git status* # 3 changed files, plus 3 migrations
$ *git add lists*
$ *git diff --staged*
$ *git commit*
----
...
'/lists/<list_id>/new_item').  We're getting pretty good at these now, so let's
knock one together quickly:
//TODO: the URL should be add_item
...
//TODO: explain correct_list, other_list
