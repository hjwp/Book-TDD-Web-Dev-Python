...
test_can_start_a_list_for_one_user
    self.check_for_row_in_list_table('1: Buy peacock feathers')
  File "/workspace/superlists/functional_tests/tests.py", line 16, in
...
Once again the FTs are doing this job, and catching a regression in our
application: because of our quick-and dirty hack where create a new list for
every single POST submission, we have broken the ability to add multiple items
to a list.  This is exactly what we have functional tests for!
