...
begins with 'test_'.  To keep things neat and tidy, let's make a folder for
...
in the book's example repo.  It's all to do with my the book's correctness
...
footnote:[Around this point, some Windows users have reported seeing (harmless,
but distracting) error messages that say `socket.error: [Errno 10054] An
existing connection was forcibly closed by the remote host`. I suspect these
may have been fixed in later releases of Selenium, but please let me know if
you see them too. obeythetestinggoat@gmail.com]
...
NOTE: In case you want to go and take a manual look at the site via
`runserver`, you should know that this change to 'models.py' won't filter
through to the "real" database at 'db.sqlite3' unless you delete it, and do a
`syncdb --noinput`, like we did in the last chapter.  The test database
`manage.py test` gets re-created with each test run, so it's fine. We'll learn
more about managing database schema updates in Chapter 12.
...
Hm, it looks like it hasn't been adjusted to the new world of Lists and Items.
