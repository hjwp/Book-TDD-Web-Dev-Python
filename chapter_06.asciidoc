...
((("functional tests/testing (FT)","cleanup", id="ix_FTcleanup", range="startofrange")))((("functional tests/testing (FT)","isolation in", id="ix_FTisolation", range="startofrange")))In this chapter we're going to address the problems we discovered at the end
...
((("Django", "functional tests (FT) in", see="functional tests/testing (FT)")))When we run 'unit' tests, the Django test runner automatically creates a brand
...
((("functional tests/testing (FT)", "running unit tests only")))Now if we run `manage.py test`, Django will run both the functional and the
unit tests:
...
((("Django", "test client")))So let's use the Django test client.  Open up 'lists/tests.py', and add a new
...
((("Red, Green, Refactor")))((("refactoring", "Red, Green, Refactor")))Time for a little tidying up.  
...
((("functional tests/testing (FT)","cleanup")))((("code", "redundant")))We're looking good. Since our new views are now doing most of the work that
...
((("URLs", "pointing forms to")))((("Django", "URLs in")))((("URLs", "in Django")))Finally, let's wire up our two forms to use this new URL.  In 'both'
...
((("Django", "view functions in", id="ix_Djangoviewfunctions", range="startofrange")))((("view functions, in Django", id="ix_viewfunctions", range="startofrange")))We need a URL and view to handle adding a new item to an existing list (
...
deploy it in the next couple of chapters.((("test isolation")))((("functional tests/testing (FT)", "isolation in")))((("Testing Goat")))((("Refactoring Cat")))
...
Test isolation::
    ...
    between each test.
//TODO: contrast with chap 19 on isolation...
