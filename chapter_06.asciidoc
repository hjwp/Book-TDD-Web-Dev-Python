...
((("functional test (FT)","cleanup after", id="ix_FTcleanup", range="startofrange")))((("functional test (FT)","isolation in", id="ix_FTisolation", range="startofrange")))In this chapter we're going to address the problems we discovered at the end
...
((("Django", "functional tests (FT) in", see="functional test (FT)")))When we run 'unit' tests, the Django test runner automatically creates a brand
...
((("LiveServerTestCase")))((("Django", "LiveServerTestCase")))Since Django 1.4 though, there's a new class called `LiveServerTestCase` which
...
((("commits")))itself.  Success! We should commit it as an atomic change:
...
same file, and show you a more sensible diff (try it without the flag!).(((range="endofrange", startref="ix_FTcleanup")))(((range="endofrange", startref="ix_FTisolation")))


Running Just the Unit Tests
^^^^^^^^^^^^^^^^^^^^^^^^^^^

((("functional test (FT)", "running unit tests only")))Now if we run `manage.py test`, Django will run both the functional and the
unit tests:
...
((("agile movement in software development")))((("Big Design Up Front")))TDD is closely associated with the agile movement in software development,
...
((("minimum viable application")))But that doesn't mean that thinking about design is outright banned! In the
...
((("YAGNI")))Once you start thinking about design, it can be hard to stop. All sorts of
...
((("REST (Representational Site Transfer)")))Representational State Transfer (REST) is an approach to web design that's
...
((("Test-driven development (TDD)", "process flowchart")))((("Test-driven development (TDD)", "new design implementation with", id="ix_TDDndimp", range="startofrange")))How do we use TDD to implement the new design? Let's take another look at
...
((("assertRegex")))http://docs.python.org/3/library/unittest.html[`unittest` documentation].
...
((("comments", "meta vs. regular")))((("meta-comments")))((("+++##+++ symbol")))Other than that, the changes are fairly self-explanatory. Let's see how they do
when we run our FTs:
...
((("Selenium", "upgrading")))NOTE: I found the FTs hung when I tried to run them today.  It turns out I
...
happening, always try upgrading Selenium!(((range="endofrange", startref="ix_TDDndimp")))


Iterating Towards the New Design
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

((("Test-driven development (TDD)", "iterating towards new design")))Being all excited about our new design, I had an overwhelming urge to dive in
...
((("Django", "test client")))((("Django", "test class in")))So let's use the Django test client.  Open up 'lists/tests.py', and add a new
...
((("Django", "URLs in")))((("URLs, in Django")))Our singleton list URL doesn't exist yet.  We fix that in 'superlists/urls.py'.
...
((("Django", "view functions in")))((("view functions", "in Django")))Nicely self-explanatory.  Let's create a dummy view function in 'lists/views.py':
...
((("Red, Green, Refactor")))((("refactoring")))Time for a little tidying up.  
...
((("templates", "separate")))((("Django", "templates")))Since the home page and the list view are now quite distinct pages,
...
((("assertTemplateUsed")))`assertTemplateUsed` is one of the more useful functions that the Django test
...
((("POST requests","sending")))We're still failing to input the second item.  What's going on here? Well, the
...
((("commits")))Hooray! We're back to where we were earlier, which means our refactoring is
...
((("Django", "test class in")))Open up 'lists/tests.py', and 'move' the
...
((("Django", "test client")))Now let's use the Django test client:
...
((("Django", "view functions in")))((("view functions", "in Django")))((("URLs, in Django")))((("Django", "URLs in")))Let's build our new URL now:
...
((("functional test (FT)","cleanup after")))((("code", "removing redundant")))We're looking good. Since our new views are now doing most of the work that
...
((("URLs", "pointing forms to")))((("Django", "URLs in")))Finally, let's wire up our two forms to use this new URL.  In 'both'
...
((("Django", "model adjustment in")))((("model adjustments", id="ix_modeladjust", range="startofrange")))Enough housekeeping with our URLs. It's time to bite the bullet and
...
((("Django", "foreign key relationship")))((("foreign key relationship")))How do we give our `Item` a list attribute?  Let's just try naively making it
...
((("migrations", "deleting")))That'll need a migration too.  Since the last one was a red herring, let's
...
(((range="endofrange", startref="ix_modeladjust)))And we can cross out another item on the to-do list:
...
((("Django", "URLs in")))((("URLs", "in Django")))((("URLs", "distinct")))What shall we use as the unique identifier for our lists?  Probably the
...
syntax later in the book too.((("string substitutions")))

Running the unit tests gives an ((("PRIMARY ENTRY HERE", "SECONDARY ENTRY HERE")))expected 404, and another related error:
...
((("URLs", "capturing parameters in")))((("parameters", "capture group")))((("capture group")))It's time to learn how we can pass parameters from URLs to views:
...
((("Django", "view functions in", id="ix_Djangoviewfunctions", range="startofrange")))((("view functions", id="ix_viewfunctions", range="startofrange")))We need a URL and view to handle adding a new item to an existing list (
...
((("greedy regular expressions")))That's a little strange. We haven't actually specified a URL for
...
((("URLs, in Django")))((("Django", "URLs in")))Now we've got our expected 404, let's add a new URL for adding new items to
existing lists:
...
That's the tests passing.(((range="endofrange", startref="ix_Djangoviewfunctions")))(((range="endofrange", startref="ix_viewfunctions")))
...
((("URLs, in Django")))((("Django", "URLs in")))Now we just need to use this URL in our 'list.html' template.  Open it up and
adjust the form tag...
...
((("Testing Goat")))Irritatingly, the Testing Goat is a stickler for tying up loose ends too, so
we've got to do this one final thing.

((("commits")))Before we start, we'll do a commit--always make sure you've got a commit
...
((("superlists")))'superlists/urls.py' is really meant for URLs that apply to your
...
deploy it in the next couple of chapters.((("test isolation")))((("functional test (FT)", "isolation in")))((("Testing Goat")))((("Refactoring Cat")))
