...
how you've split work between different appps!
...
the list of things we import from `django.test`, and add a new test class
called `ListViewTest`.  Then copy the method called
...
class ListViewTest(TestCase):
    ...
    def test_list_view_displays_all_items(self):
        ...
        client = Client() #<1>
        response = client.get('/lists/the-only-list-in-the-world/') #<1>
...
<1> Instead of calling the view function directly, we instantiate a test client
...
<2> Instead of using the slightly annoying `assertIn` / `response.content.decode()`
dance, Django provides the `assertContains` method which know how to deal with
responses and the bytes of their content.

NOTE: watch out for trailing slashes in URLs, both here in the tests and in
'urls.py' -- they're a common source of bugs.

TOOD: discuss the impact on speed of unit tests from using the Django test client
TOOD: switch to using the built-in test client, `self.client` (added in v1.5)
...
Now let's use the Django test client.  It's an attribute of the Django `TestCase`:

TODO: include note that use of Django test client is considered controversial.
...
[source,diff]
----
@@ -4,5 +4,5 @@ from django.test import Client, TestCase
...
ERROR: test_list_view_displays_all_items (lists.tests.ListViewTest)
 ---------------------------------------------------------------------
Traceback (most recent call last):
  File "/workspace/superlists/lists/tests.py", line 61, in
...
ERROR: test_saving_a_POST_request (lists.tests.NewListTest)
 ---------------------------------------------------------------------
Traceback (most recent call last):
  File "/workspace/superlists/lists/tests.py", line 79, in
...
----
  File "/workspace/superlists/lists/views.py", line 9, in new_list
    Item.objects.create(text=request.POST['item_text'])
----
...
checks that we use the correct template, and pass it the correct List 
object, and then another test that does a sense-check that the right 
things actually appear as rendered by the template:
...
Running the unit tests gives an expected 404:
...
----
ERROR: test_list_view_displays_items_for_that_list (lists.tests.ListViewTest)
[...]
...
class NewItemTest(TestCase):
    ...
    def test_redirecst_to_list_view(self):
        other_list = List.objects.create()
        correct_list = List.objects.create()
...
object for us, to help with testing. That gives us `KeyError: 'list'` because
we're not passing list into the template.  It actually gives us an opportunity
