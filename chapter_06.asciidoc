...
for it to be a valid Python module (i.e., one with a '__init__.py' in it):
...
$ *git diff --staged -M*
$ *git commit*  # msg eg "move functional_tests an app, use LiveServerTestCase"
----
...
http://bit.ly/1slNmsg['Dive into Python'] has a good overview, if you want to go look them up quickly.  We'll see an alternative
...
Now that we've got our expected 404, let's add a new URL for adding new items to
existing lists:
...
Rerun the unit tests to check that everything worked.  When I did it, I
