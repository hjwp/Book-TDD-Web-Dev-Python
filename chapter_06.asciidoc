...
//TODO: this will change in 1.7. vcheck?
...
That now passes.  We're looking good. Can we remove the old 
+if request.method == 'POST'+ code from `home_page`?
...
NOTE: I'm using the variable `list_` to avoid "shadowing" the Python built-in
`list` function
...
//TODO: good spot for another bit on reading tracebacks?

That gets us down to two failing tests, both on tests that try to POST
to our new_list view. Decoding the tracebacks, the area comes from a line
in the view:
...
[source,python]
----
    def test_list_view_passes_correct_list_to_template(self):
        other_list = List.objects.create()
        correct_list = List.objects.create()
        response = self.client.get('/lists/%d/' % (correct_list.id,))
