...
in a `./manage.py` +shell+ if you like:
...
*******************************************************************************
((("JSON fixtures")))((("fixtures", "JSON fixtures")))When we pre-populate the database with test data, as we've done here with the
...
Test fixtures::
    ((("test fixtures", "defined")))Test fixtures refers to test data that needs to be set up as a precondition
...
Avoid JSON fixtures::
    ((("JSON fixtures")))((("dumpdata command")))((("loaddata command")))Django makes it easy to save and restore data from the database in JSON
