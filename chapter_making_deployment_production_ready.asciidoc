...
<1> We say we'll use an environment variable called `DJANGO_DEBUG`
    to switch debug mode off
...
accidentally forgetting to set one.  The end result is that you either
run with all three set, for production, or none of them, for dev.
...
elspeth@server:$ *export DJANGO_DEBUG=false DJANGO_SECRET_KEY=abc123*
...
Environment=DJANGO_DEBUG=false  <4>
Environment=SECRET_KEY="1*b84qe7z%4v)(_ji01k^_xx2tkhs062zpsza9gpzl3rxdfhfv"  <4>
...
Environment=SECRET_KEY="1*b84qe7z%4v)(_ji01k^_xx2tkhs062zpsza9gpzl3rxdfhfv"
