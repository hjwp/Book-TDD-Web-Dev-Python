...
We do our usual dance of committing locally, then push it up to github, and
restart gunicorn:

[subs="specialcharacters,quotes"]
----
$ *git commit -am "set DEBUG=False if SITENAME is populated"*
$ *git push*
----
...
And we use a test run to reassure ourselves that things still work:
...
<3> `Environment` sets our environment variable
...
server {
    ...
    location /static {
        alias /home/elspeth/sites/HOSTNAME/static;
    }
...
----
[Unit]
Description=Gunicorn server for HOSTNAME

[Service]
Restart=on-failure
User=elspeth
WorkingDirectory=/home/elspeth/sites/HOSTNAME/source
Environment=SITENAME=HOSTNAME
ExecStart=/home/elspeth/sites/HOSTNAME/virtualenv/bin/gunicorn \
    --bind unix:/tmp/HOSTNAME.socket \
...
a new site, by doing a find and replace on `HOSTNAME`.
...
* replace HOSTNAME with, e.g., staging.my-domain.com
