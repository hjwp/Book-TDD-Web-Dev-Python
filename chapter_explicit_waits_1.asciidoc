...
Ensuring Test Isolation and Managing Global State::
    ((("testing best practices", "Selenium/functional tests", "ensuring test isolation isolation")))Different tests shouldn't affect one another.  This means we need to
...
Avoid "voodoo" sleeps::
    ((("testing best practices", "Selenium/functional tests", "avoiding &quot;voodoo&quot; sleeps")))Whenever we need to wait for something to load, it's always tempting to
...
Don't rely on Selenium's implicit waits::
    ((("testing best practices", "Selenium/functional tests", "using explicit waits")))Selenium does theoretically do some "implicit" waits, but the
