...
In a web app, there are two places you can do validation: on the client-side
...
do validation in Django. One is at the model level, and the other is higher up
...
So why is the test still failing?  Well, for
...
NOTE: So did I break the rule about never refactoring against failing tests?
    ...
    failingfootnote:[If you really want a "clean" test run, you could add a
...
I always like to push my validation logic down as low as possible.  The
