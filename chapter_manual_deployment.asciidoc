...
((("deployment", "testing using staging sites", id="DEPstage09")))It
's time to deploy the first version of our site and make it public.  They say
...
((("staging sites", "adapting functional tests for", id="SSadapt09")))Let
's adapt our functional tests slightly so that it can be run against
...
((("staging sites", "domain names")))((("domain names")))We
're going to need a couple of domain names at this point in the book--they
...
- Using a ((("Platform-As-A-Service (PaaS)")))Platform
-As-A-Service (PaaS) offering like Heroku, OpenShift, or
...
((("Nginx", "installation")))We
'll need a web server, and all the cool kids are using Nginx these days,
...
((("Nginx", "confirming operation of")))To
 confirm  this works, we can rerun our functional tests and see that their
...
((("", startref="SSserver09")))((("", startref="seerver09")))Progress
!  Give yourself a pat on the back, and maybe a nice cup of tea
...
((("staging sites", "manual code deployment", id="SScode09")))The
 next step is to get a copy of the staging site up and running, just to
...
((("requirements.txt")))((("virtual environment (virtualenv)", "creating manually")))To
 "save" the list of packages we need in our virtualenv, and be able to
...
TIP: Depending on your firewall configuration, you may even be able to manually
    ...
    pass:[<em>http://your.domain.com:8000</em>].
