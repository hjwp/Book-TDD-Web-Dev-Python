...
remember to substitute it in in all the places I've hard-coded it below.
...
[subs="specialcharacters,macros"]
----
$ pass:quotes[*STAGING_SERVER=superlists-staging.ottg.eu ./manage.py 
...
[subs="specialcharacters,macros"]
----
$ pass:quotes[*STAGING_SERVER=superlists-staging.ottg.eu:8000 ./manage.py 
    test functional_tests --failfast*]
...
running `curl` on the server itself (you'll need a second SSH shell onto your server, so as not to interrupt the
