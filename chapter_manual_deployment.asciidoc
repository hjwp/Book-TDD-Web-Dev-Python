...
NOTE: 'Update on 2017-11-21' -- I have just pushed up a bit of a simplification
    ...
    folder are now in the same folder as 'manage.py'.  Adjust your 'settings.py'
    accordingly.  Check the reference repo on 
...
Traceback (most recent call last):
  File "...python-tdd-book/functional_tests/tests.py", line 49, in
...
Traceback (most recent call last):
  File
"...python-tdd-book/functional_tests/tests.py", line 86, in
...
----
elspeth@server:$ *pwd*
/home/espeth/sites/staging.superlists.com
...
that.  We can actually do everything we want to by directly calling the
versions of Python, pip, and the other executables in the virtualenv's 'bin'
directory, as we'll soon see.
...
// (note this is only working because nginx setting HOST=localhost)

That looks like it's running happily.  We can Ctrl-C it for now.


More progress!  We've got a system for getting code to and from the server
