...
((("Test-Driven Development (TDD)", "philosophy of", "YAGNI")))((("YAGNI (You ain&#x8217;t gonna need it!)")))Once you start thinking about design, it can be hard to stop. All sorts of
...
((("Representational State Transfer (REST)", "inspiration gained from")))((("Model-View-Controller (MVC) pattern")))We have an idea of the data structure we want--the Model part of
...
((("Test-Driven Development (TDD)", "overall process of")))((("multiple lists testing", "incremental design implementation")))How do we use TDD to implement the new design? Let's take another look at
...
<1> `assertRegex` ((("assertRegex")))is a helper function from `unittest` that checks
    ...
    new REST-ish design has been implemented. ((("unittest module", "documentation")))Find out more in the http://docs.python.org/3/library/unittest.html[`unittest` documentation].
...
((("multiple lists testing", "iterative development style")))((("iterative development style")))Being all excited about our new design, I had an overwhelming urge to dive in
...
NOTE: Another way of thinking about it is as a problem-solving pass:[<span class="keep-together">technique</span>]: our
...
the list table; it's because URL pass:[<span class="keep-together"><em>/the-only-list-in-the-world/</em></span>] doesn't exist
yet!
...
((("multiple lists testing", "URL mappings", id="MLTurl07")))((("URL mappings", id="url07")))Open up 'lists/tests.py', and add a new test class called `ListViewTest`.  Then
...
<1> ((("assertContains method")))Here's a new helper method: instead of using the slightly annoying
...
TIP: ((("troubleshooting", "URL mappings")))Watch out for trailing slashes in URLs, both here in the tests and in
...
((("debugging", "of functional tests")))((("functional tests (FTs)", "debugging techniques")))((("POST requests", "debugging")))((("HTML", "POST requests", "debugging")))It's failing when we try to add the second item.  We have to put our debugging
...
((("multiple lists testing", "refactoring")))((("refactoring")))((("Red/Green/Refactor")))Time for a little tidying up.  
...
((("multiple lists testing", "separate list viewing templates", id="MLTseparate07")))((("templates", "separate list viewing templates", id="TMPseparate07")))Since the home page and the list view are now quite distinct pages,
...
((("", startref="MLTseparate07")))((("", startref="TMPseparate07")))It may feel like we haven't made much headway since, functionally, the site
...
((("multiple lists testing", "list item URLs", id="MLTlist07")))((("URL mappings", id="urlmap07a")))((("list items", id="list07")))Where are we with our own to-do list?
...
NOTE: Do you remember how we split this out into two tests earlier? If we
...
((("", startref="MLTlist07")))((("", startref="urlmap07a")))((("", startref="list07")))And we can cross out an item on the to-do list:
...
((("multiple lists testing", "model adjustments", id="MLTmodel07")))Enough housekeeping with our URLs. It's time to bite the bullet and
...
by assigning it as its `.list` property.  We check that the list is properly
...
will compare themselves by checking that their primary key (the `.id` attribute)
is the same.
...
I know, I feel the same.  ((("Test-Driven Development (TDD)", "philosophy of", "working state to working state")))((("working state to working state")))((("Testing Goat", "working state to working state")))The step-by-step approach, in which you go
...
((("", startref="MLTmodel07")))Sure enough, it gets all the way through to where we were before.  We haven't broken
...
((("multiple lists testing", "adding items to existing lists", id="MLTaddingitems07")))What shall we use as the unique identifier for our lists?  Probably the
...
new list it just pass:[<span class="keep-together">created</span>]:
...
((("", startref="MLTaddingitems07")))What about the functional tests?  We must be almost there?
...
((("regular expressions")))((("greedy regular expressions")))That's a little strange. We haven't actually specified a URL for
...
((("HTML", "POST requests", "redirect following")))((("POST requests", "redirect following")))((("permanent redirect (301)")))Django has some built-in code to issue a permanent redirect (301) whenever
someone asks for a URL which is 'almost' right, except for a missing slash.
In this case, pass:[<span class="keep-together"><em>/lists/1/add_item/</em></span>] would be a match for `lists/(.+)/`, with
...
((("response.context")))We've got our new view and URL for adding items to existing lists; now we just
...
<2> `.item_set` is ((("reverse lookups")))called a 
...
'superlists/urls.py' is ((("multiple lists testing", "refactoring using URL includes", id="MLTrefacturl07")))really meant for URLs that apply to your
...
YAGNI::
    ((("Test-Driven Development (TDD)", "philosophy of", "YAGNI")))((("YAGNI (You ain&#x8217t gonna need it!)")))You ain't gonna need it!  Avoid the temptation to write code that you 
