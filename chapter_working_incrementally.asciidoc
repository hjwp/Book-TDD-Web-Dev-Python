...
((("Test-Driven Development (TDD)", "adapting existing code incrementally", id="TDDadapt07")))Now let's address our real problem, which is that our design only allows for
...
((("small design", id="small07")))Let's have a think about how we want support for multiple lists to
...
((("", startref="small07")))In summary, our scratchpad for this chapter looks something like this:
...
<1> `assertRegex` is a helper function from `unittest` that checks
    ...
    new REST-ish design has been implemented. Find out more in the http://docs.python.org/3/library/unittest.html[`unittest` documentation].
...
Split work out into small, achievable tasks::
    ((("Test-Driven Development (TDD)", "philosophy of", "split work into smaller tasks")))((("small design")))Sometimes this means starting with "boring" work rather than diving
