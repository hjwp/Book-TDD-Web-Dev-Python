...
<1> ((("assertContains method")))Here's
...
`ForeignKey` takes the class you want to create the relationship to, and then two
extra arguments:  `default` you can probably guess, and `on_delete` specifies what
to do if we delete the related List object.  `CASCADE` says we also want to delete
the related items, which seems sensible in our case.
...
We use the special syntax `<parameter>`, putting angle brackets around a name
for our parameter, and the captured text will get passed to the view as an
argument.
...
//TODO: just do import lists.views
