...
((("layout and style", "rows and columns")))((("Bootstrap", "rows and columns in")))Finally, let's actually use some of the Bootstrap magic! You'll have to read
...
* How to tell when a URL request is for a static file, as opposed to for some
...
The rest of the settings we will add to this section are all to do with the second item:
