...
test_layout_and_styling
    delta=5
AssertionError: 125.0 != 512 within 5 delta
...
While we're using the Django development server (`manage.py runserver`), or
`LiveServerTestCase`, we can rely on Django to magically find static files for
us -- it'll just look in any subfolder of one of our apps called 'static'.
footnote:[LiveServerTestCase is set to change in Django 1.7, and stop magically
detecting static files.  There will be an alternative called 
https://docs.djangoproject.com/en/dev/ref/contrib/staticfiles/#django.contrib.staticfiles.testing.StaticLiveServerCase[StaticLiveServerCase]]
...
starting to look a little better laid-out, as in <<list-page-centered>>.

[[lists-page-centered]]
.Our site looking a little better...
...
Let's see if we can do even better, using some of the other tools in Bootstrap's
panoply.
...
[subs="specialcharacters,quotes"]
----
$ *git status* # shows changes to base.html, list.html, and untracked lists/static
$ *git add lists*
...
`collectstatic` has also picked up all the css for the admin site. It's
one of Django's powerful features, and we'll find out about it later,
