...
TODO: by_tag name can be confused if the csrf token is before the real
input. change locators.
...
[subs="specialcharacters,quotes"]
----
$ *wget -O bootstrap.zip https://github.com/twbs/bootstrap/releases/download/v3.0.3/bootstrap-3.0.3-dist.zip*
$ *unzip bootstrap.zip*
$ *mkdir lists/static*
...
NOTE: At this point, some Windows users have reported seeing (harmless, but
distracting) error messages that say `socket.error: [Errno 10054] An existing
connection was forcibly closed by the remote host`. I suspect these may have
been fixed in later releases of Selenium, but please let me know if you see
them too. obeythetestinggoat@gmail.com
//TODO: check whether this is still happening, adjust msg
...
inside their app folders, and copy them into a single location, ready for
deployment. This is what the `collectstatic` command is for.
